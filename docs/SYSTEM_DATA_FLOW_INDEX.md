...
- **The Account hub's EXPENSES column + the per-scope drill-down (owner request 2026-09-21: "in teh
  finance accout module add expenses column also and let each store be drilled down to get more
  details"):** `/accounts` (the finance hub) By Company / By Store tables gained an **Expenses**
  column between Gross Profit and Net Income, and every scope row EXPANDS into the expense detail
  behind that number. NOTHING new derives money. **ONE HOME, and the duplicate that already existed
  was folded into it:** `account/analysis.pl_totals` owns "headline numbers from a stored statement
  payload" and now also owns **`expenses` = Σ `analysis.EXPENSE_SECTIONS`** (`('opex','other')` — the
  statement sections `engine._assemble` subtracts from gross profit, i.e. `statement_engine
  .PL_SECTIONS` below GP). `GET /account/overview/{period}` **dereferences** it — it previously
  re-summed the revenue sections itself, a second copy `pl_totals`' own docstring named — and so does
  the narrative helper `router._consolidated_pl`. Identity, everywhere: **gross_profit − expenses ==
  net_income**; the column therefore IS the P&L's expense total for that scope+period, and a
  reclassification between expense lines (or a new expense line) moves both in the same recompute.
  **The drill-down REUSES the canonical read** `GET /account/pl/{period}?scope=<scope_key>` — the same
  endpoint `/accounts/pl` fetches, unfiltered — so the panel renders the stored snapshot (expense
  lines, their stored `detail`, the ruling-K3(b) declared-zero notes) and links on to the full P&L /
  Balance Sheet / Cash Flow for that scope. NO new endpoint, no new table, no migration. Display
  helpers: `frontend/src/app/(platform)/accounts/_components/scopeFinancials.ts` (pure; the panel
  TIES OUT its detail against the column and SAYS SO when they differ — a check, never a source).
  **Honesty:** a scope with no computed P&L publishes NO `expenses` key and renders "not reported"
  (export cell BLANK); a P&L reporting no expenses renders a measured `$0.00`. Proofs:
  `backend/harness_account_expenses_one_home.py` (42 checks — runs the REAL `engine._assemble` and
  `statement_filter.aggregate`; **wired into CI** (`carrier-vocab-guard` job) as the LOCK: a caller
  that stops dereferencing the home, a below-GP section missing from `EXPENSE_SECTIONS`, or a
  frontend mirror that drifts, FAILS THE BUILD) + `frontend/prove_accounts_expenses_column.mjs`
  (53 checks). Live reconciliation 2026-09-21: house org August 2026 — 44 scopes, hub column ==
  P&L expense total on every one (consolidated $472,993.92); second org July 2026 — 24/24
  (consolidated $233,461.45); 0 mismatches, identity held on all 68.
