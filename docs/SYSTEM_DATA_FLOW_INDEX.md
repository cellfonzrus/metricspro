...
- **Tolerant .xlsx reading (owner blocker 2026-09-20, `commcalc/xlsx_tolerant.py`):** both upload readers —
  ...
  `xlsx_tolerant.read_excel` (via `router._xlsx_read`): the stock `pd.read_excel` first, and ONLY on the
  KeyError naming a missing archive member (`xl/sharedStrings.xml` stored as a case variant, or declared but
  never written by an inline-string exporter) the ZIP is repaired to match what it declares and re-read. A file
  that reads first time is byte-identical. A repair is NOTED — `xlsx_repair` on the upload trace result and on
  the intake's `detect` — never silent; an HTML / XML / text file named .xlsx is refused with a sentence
  (`UnreadableWorkbook`). Proof `harness_xlsx_tolerant.py` (23) + `harness_onboarding_intake_c.py` §H. Other
  `pd.read_excel` sites (the multisheet feed reader, the X-report parser, the MA wizard) are unchanged.
- **Sales BY INVOICE with the tender types, and the cash-collected basis (§30.13, 2026-09-21):** the invoice-level export is its
  own report kind (`sales_by_invoice` → `raw_sales_invoice` + one row per invoice × declared tender column in
  `raw_sales_invoice_tender`, mig `1012`, NOT applied); the tender columns are DECLARED at intake step **2.5b** and classed through
  the ONE tender vocabulary (`closing.router.TENDER_VOCAB` / `tender_class`; `CANON_TENDERS` derived from it); remembered as
  `closing_tender_map` rows (report `invoice`); the Stage-4 verify shows the split per store-day beside the X-report and Σ tax; what
  Cash Collected / the cash-card and deposit recons READ per store-day is the per-org `tender_basis` (`storeops.tenants.closing_tender_basis`,
  house default `x_report` — byte-identical; `invoice`; `x_report_else_invoice`) through ONE resolver
  (`closing.router._tender_split_by_store` = `_xreport_tenders_by_store`). Lock `harness_tender_vocab_lock.py`; proof `harness_sales_by_invoice.py`.
- **Which lines are activations / upgrades / BYOD / ports — and phones, bill payments (§3, §30.12, 2026-09-21):**
  ONE predicate `commcalc/line_class.activation_class(row, rules)` over the org's rules in
  `accessory_config.activation_details_rules` (`fields` + `tokens` + `exact`; house defaults = the retired
  `classify_contract_type` / `_exec_act_class` / `contract_type_map` chain, byte-identical), dereferenced by the
  pay path, the plan engine, the Sales Report / Exec MTD / Targets aggregation, the closing recon, what-if,
  sales comparison and the event register; mapped at intake step **2.5a** (`GET/PUT /onboarding/intake/line-class`)
  from the file's own words with the count each rule would classify; a landed sales export is not verified
  while no line classifies unless attested; the Exec MTD `landing.classified` banner is the way back. Lock:
  `harness_line_class_lock.py`; proof + money pin: `harness_line_class.py`; intake proof: `harness_onboarding_intake_d.py`.
  Since the 2026-09-21 addendum the save is GUARDED (a tenant word naming ≥80% of the lines is refused unless attested by
  name; what is saved is re-validated on GET / commit / Exec MTD), house tokens never leak onto tenant-declared fields, and
  the step seeds from the proposal only (`intake/line-class-logic.ts`, proof `frontend/prove_line_class_step.mjs`).
...
- **Classification single source (2026-09-21, owner: "88 txns but not a break up in to activations and
  upgrade etc, also nothing on exec mtd"):** `line_class.activation_class(row, rules)` →
  `'activation'|'upgrade'|'byod'|'port'|'hardware_only'|None`, and `line_class.classify_line(row, rules)` /
  `calculator.classify_line` → the bucket `'byod'|'upgrade'|'premium'|None` every surface sums on (activation
  + port = premium). THE predicate reads the org's RULES — `accessory_config.activation_details_rules`
  (mig 313 JSON, extended: `fields` = which columns carry the type, default `['contract_type']`; `tokens`
  per class, matched by CONTAINS over every configured field; `exact` = the mig-213 `contract_type_map`
  absorbed; `auto_activation_tokens` = the mig-213 non-phone opt-in; `hints` / `metric_hints` = the
  suggestion vocabulary) resolved once by `_accessory_config_uncached` → `acfg['line_rules']`
  (`line_class.resolve_rules`, a TENANT-authored exec_metric_config `activation` row honoured as a legacy
  layer for port/byod/upgrade tokens the home does not declare). HOUSE DEFAULTS = the retired
  `calculator.classify_contract_type` (PREMIUM_ACT / `_PREMIUM_KEYS`) + `router._exec_act_class` (the Port
  sub-split) + `router._resolve_ct_bucket` chain, BYTE-IDENTICAL — `harness_line_class.py` §B replays them
  verbatim over 1,500+ spellings scraped from the seeds / migrations / fixtures (the money pin).
  `classify_contract_type(ct)` survives only as a harness alias (a bare value or a row); no app module calls
  it — `harness_line_class_lock.py` fails the build on a caller, a second token list, a second reader /
  writer of the column, or a surface that stops rendering the way back. Precedence: exact-mapped value
  wins (`none` excludes) → hardware_only → byod → upgrade → the activation gate → `port` when a port token
  hits (and no byod / upgrade token does). `None` = accessory / bill payment / feature line. EXCUSED by name
  (their own question, their own feed): `activation_bucketing.activation_details_bucket` (the carrier-portal
  Activation-Details basis, §15), `asset/router._promo_type` (the hotsheet promo-column taxonomy),
  `sale_installment_engine` (the residual category chain, §7). See §30.12 for the intake step that maps it.
...
**THE SECOND SOLD-SOURCE — the ACTIVATION feed (owner 2026-09-20, §30.8):** `reconcile(..., activation_rows=)`
takes the activation feed as the platform already models it (`_cr_resolve_activation_details`, per device, now
carrying `mdn`) and adds the finding `activated_not_rung_out` — on hand, in the activation feed, and either no
sale line names the unit or its sale was refunded back into stock (`sale_state`). Pairing: the activation's own
serial (`device_key`), else its mobile number THROUGH a sale line (`mobile_number_via_sales`, same line or same
transaction, stated per row); anything else is UNPAIRABLE with the reason (no key and no number; no sale line
carries the number; the number appears on lines for several devices — candidates listed). An activation on a
sales hit is EVIDENCE on the one `sold_not_cleared` row, never a second row. `GET /commcalc/inventory-sold-recon`
(`?period=`) returns `activations` + `basis.sources` (per side: table, rows, read_ok, what it pairs on, whether
the feed carries a device key / a mobile number); the page's second section lists the rows with their evidence
...
- **Manual upload of a settlement export (onboarding intake Stage C, §30.8, 2026-09-20):** the SAME
  `normalize_settlement` + `store_settlement` the sweep runs, landed under a deterministic UPLOAD `source_id`
  (`uuid5(org, portal, role)`) with `report_key='intake_upload'`, the role from the source row's override / the
  portal's house default / the person's pick — settlement grain only; merchant ids through the mig-902 map (an
  assign at 2.4 writes it); an overlap with a day another source already landed is REFUSED (the card recon would
  sum both). `closing/external_credit_recon` reads the rows unchanged (pinned in `harness_onboarding_intake_c.py` §C).
...
| `commcalc.commission_bucket` (mig `1009`, **NOT applied**) — THE BUCKET REGISTRY: one row per bucket per org (`key` = `commission_ledger.category`, `label`, `kind` earned\|deduction, `sort_order`, `is_active`, `hint_words`, `pl_line_key`, `is_builtin`), house org = defaults (the five mig-071 buckets + `chargebacks` / `vendor_fee` / `misc_charges`), a tenant's row overrides per key | `POST /commcalc/commission-buckets` (admin-gated; key/kind/P&L line validated — `pl_line_key` must be a key of `account/coa.PL_SPEC`) · `DELETE /commcalc/commission-buckets/{key}` (never a column-backed key, never a key ledger lines use) · the 1009 house seed | `commission_ledger.load_buckets_meta` (both reads org-scoped) → `classify_line` / `build_row` / `summarize`, `onboarding_intake.suggest_buckets` (the `hint_words`) / `bucket_totals` / `rules_for_assignments`, `/commission-ledger/*` payloads (`buckets`, `bucket_meta`), `/commission-category-map` (the picker), `/onboarding/intake/*`. **The P&L link is RECORDED here, not booked** — finance's `coa.build_inputs` does not read the ledger (§30.7) |
| `commcalc.report_kind` (mig `1010`, **NOT applied**) — THE REPORT-KIND REGISTRY: one row per report kind per org (`key`, layman `label` / `what_in_it` / `recognisable_columns` / `source_hint`, `applies_to_pos[]` / `applies_to_carrier[]` codes ({} = any), `defined_by` house\|tenant + `defined_by_org`, `statement_type`, `landing`, `layout`, `signature_fields[]` (TARGET_FIELDS names), `requires_columns[]` / `excludes_columns[]`, `upload_types[]`, `custom_sheet_label`, `sort_order`, `is_active`); house org = the seed (mirror `report_kinds.HOUSE_KINDS`), a tenant row overrides per key | the 1010 house seed · `report_kinds.define_kind` (a 'something else' confirmed at the intake → a house row `defined_by='tenant'` under the tenant's carriers) | `report_kinds.load_registry` → `merge_rows` → `visible_kinds` (THE visibility function; frontend twin `carrier-scope.reportKindsVisible`) → `GET /commcalc/report-kinds` → every upload surface through `lib/report-kinds.useReportKinds`; `detect_report_kind`; `kind_key_for` (the learn hook). §30.9 |
| `commcalc.report_signature` (mig `1010`, **NOT applied**) — header fingerprints CONFIRMED at an intake: `org_id`, `fingerprint` (normalised ORDERED header names, '\|'-joined), `report_kind_key`, `statement_type`, `layout`, `header_count`, `confirmations`, `first/last_confirmed_at`, `house_copy`. **Header names only — never a cell value, filename, store, rep or customer string** | `report_kinds.learn_signature` ← `router._intake_learn_signature` on a CONFIRMED `/onboarding/intake/commit` (org row + house copy, +1 per confirm) | `load_signatures` ({org, house}) → `detect_report_kind` (exact fingerprint first, "confirmed N times"; a fingerprint under two kinds = ambiguous → ask) → `POST /report-kinds/detect`; `confirmations_by_kind` → the "confirmed by you N times" provenance. §30.9 |
...
| `commcalc.raw_sales_invoice_tender` (mig `1012`, **NOT applied**) — the TENDER SPLIT: one row per (invoice, DECLARED tender column) with `amount` ≠ 0 — `tender_label` (the header, verbatim), `tender_class` (`closing.router.TENDER_VOCAB` key), `keyed_manually`, `role` (`tender` \| `tax` — a jurisdiction tax column rides the same grain), store / `trans_date` / `trans_id` / salesperson, `source` = the PARENT's kind (`column_mapping.CHILD_TABLE_MAP` — the one home of "which child table") | the same landing, second `_ingest_mapped_df` call (same stamp, same slice) | `_intake_reread_invoice_tenders` (the save guarantee); **`closing.router._invoice_tenders_by_store`** — the invoice leg of THE tender resolver `_tender_split_by_store` (read by every closing cash / card recon when the org's `tender_basis` is `invoice` or `x_report_else_invoice`); the intake's Stage-4 tie-out (`_intake_invoice_tender_recon`) beside the X-report leg |
| `storeops.tenants.closing_tender_basis` (mig `1012`, **NOT applied**; NULL = house default) — THE TENDER BASIS: `x_report` \| `invoice` \| `x_report_else_invoice` — what the closing recons read as the tender split per store-day | `closing.router.put_tender_basis` (THE one writer) ← the intake's 2.5b answer ("Do you also upload a daily X-report?") at commit; read back | `closing.router.tender_basis` / `tender_basis_info` → `_tender_split_by_store` (ONE resolver; `_xreport_tenders_by_store` = its totals) → `_closing_summary_for_date` (money recon, `tender_source` per store), `deposit_recon_report`, `_pos_tenders_for_days` (cash-recon-management / pickups); every payload carries `tender_basis` and the pages state it. Lock: `harness_tender_vocab_lock.py` §e |
| `commcalc.closing_tender_map` report = `'invoice'` (mig 111's table, one more leg — 2026-09-21) — the org's confirmed tender COLUMNS: one row per class (`source_labels` = the exact headers, `match_mode` exact, priority 10) + the `keyed_manually` flag row | `router._intake_write_tender_map` at the invoice commit (deletes report='invoice' rows only; read back through the resolver) | `_intake_tender_resolver` → `tender_config.make_resolver(maps, 'invoice', closing.router.tender_class, TENDER_CLASSES)` — "your earlier choice" on the next analyze; the X-report / sales legs never read report='invoice' rows (`make_resolver` filters per leg; `_xreport_config_labels` too) |
| `commcalc.pos_tender_summary` (POS X-REPORT — the AUTHORITATIVE cash/card tender split; `store` is an ADDRESS STRING, not a code) | the X-report import — `commcalc/router._parse_xreport_detail`; the recognized tender vocabulary is the built-in `_XR_TENDERS` ∪ the tenant's `closing_tender_map` labels ∪ **anything `closing/_canon_tender` can place** (`_xr_canon_known`, §23h — one vocabulary with the recon axis), and `tender_class` comes from the single `_xr_tender_class` rule shared by BOTH parser paths | `closing/router._xreport_tenders_by_store` → `_addr_resolver` (alias > storeops MASTER > store_mapping > unambiguous house number; §23b) → every closing cash/card recon. Also an OBSERVED source of `GET /commcalc/store-resolution` since 2026-09-07. **Also WRITTEN by the onboarding intake's X-report kind through the SAME `_xreport_land_rows` helper the upload handler calls (§30.8; the sheet names resolve through the alias the intake writes, which `_addr_resolver` reads first)** |
...
| `GET /commcalc/inventory-sold-recon` (`?period=`; is an on-hand unit already sold, or ACTIVATED but never rung out — READ-ONLY, books nothing, org-scoped on all three sides) | `router.inventory_sold_recon_endpoint` → PURE `commcalc/inventory_sold_recon.reconcile`, keyed by `device_cost_recon.device_key`; activations via `_intake_activation_rows` → `_cr_resolve_activation_details` | §11a — PRESENCE, distinct from `/device-cost-recon` (whose number is right) and `/account/inventory-recon` (dollars per store); §30.8 the activation source, `activations` + `basis.sources` per side. Proof `harness_inventory_sold_recon.py` (43) + `harness_onboarding_intake_c.py` §F |
| `GET /commcalc/billpay-extract/{period}` (`?store=`) — **THE BILL PAYMENTS REPORT**: bill payments EXTRACTED from the sales export by the ONE `bill_payment` rule, each line with its match provenance, Σ + count per store-day, the rule's tokens that matched nothing, beside the carrier's own bill-pay feed when present (both figures + difference). READ-ONLY, org-scoped | `router.billpay_extract_report` → PURE `commcalc/billpay_extract` over `exec_metric_defs.line_match` (`_exec_metric_config`), `_sales_rows_union`, `_billpay_processor_by_store_day` (the carrier side) | §30.8; page `commcalc/bill-payments/page.tsx`. Pinned = `_sales_cell_agg`'s `bill_qty`/`bill_amt`. Proof `harness_onboarding_intake_c.py` §E |
| `GET /commcalc/onboarding/intake/line-class?instance_key=` (2.5a, read-only: the instance's LANDED slice re-read as the commit re-read it → `line_class` block: current rules + what they classify, `gate_open`, the proposal per class with counts, the metric buckets' match / proposal) · `PUT /commcalc/onboarding/intake/line-class` (`fields`, `tokens`, `metric_rules`, `no_activations`, `broad_ok`, `by` → **THE GUARD first** (2026-09-21 §30.12 addendum): the rules that WOULD be in force resolved through `_line_rules_resolve` and measured over the landed slice (or the kept file's frame via `_intake_draft_frame` when nothing is landed) — a word naming ≥ `BROAD_RATIO` of the lines → **400 `{message, refused[], basis, rows, attest_with}`, nothing written** unless `broad_ok` attests it by name; then `put_exec_metric_config` per LINE bucket (first) + `put_accessory_config(activation_details_rules)`, then the slice RE-COUNTED through the predicate onto `verified_numbers.activation_classes` / `activation_gate`; status verified when the landing was ok and the gate is closed AND nothing is refused; gated like every Classification edit; the GET block carries `refused` / `rules_ok` / `refusal_note` and `rules.house_fill`, never a hint list) | `router.onboarding_intake_line_class` / `onboarding_intake_put_line_class` → `_intake_landed_slice` / `_intake_draft_frame` → `_intake_reread_sales` / `_intake_prepare` → `_line_rules_resolve` → `line_class.token_shares` / `refused_tokens` → `_intake_line_class_block` (`line_class.suggest_rules` / `suggest_metric_rules` / `count_classes`) | §30.12; proofs `harness_onboarding_intake_d.py` (49), `frontend/prove_line_class_step.mjs` (17) |
...
| `POST /commcalc/onboarding/intake/analyze` (line-level sales kinds now carry `line_class` — the 2.5a block over the frame that would land) · `/commit` (now RE-COUNTS the landed slice: `verified_numbers.activation_classes` + `activation_gate` + `landing_ok`; the response's `ok` = the landing re-read matched, `verified` = and the activation gate closed — else the stage row is `needs_input` naming step 2.5a; the `attestation` JSON accepts `no_activations`) | `_intake_prepare_stage2` → `_intake_line_class_block`; `_intake_commit_stage2` → `line_class.count_classes` + `onboarding_intake.activation_gate` | §30.12 |
...
| `POST /commcalc/commission-ledger/import` · `POST /commcalc/commission-ledger/analyze` (classify a statement into the org's registry buckets — the five column-backed + every registry bucket; **optional `statement_type` Form — else the type is read off the template's `source_report`**) | `router.commission_ledger_import` / `commission_ledger_analyze` → **`_ledger_mapping_key`** (§30.10) → `_ledger_source_rules(report_key=)` → `_ledger_convention(hdr_rules)` → `_ledger_footer_drop(report_key=)` → `commission_ledger.build_row`/`summarize`. Payloads carry `report_key`, `statement_type`, `rules_source`, `convention`, `convention_meta`, `footer_rows_dropped` | §15 canonical ledger, §25.12, §30.10. Proof `harness_statement_type_mapping.py` §D |
...
| Canonical commission buckets — the org's REGISTRY (`commcalc.commission_bucket`, mig 1009; house defaults Commission / Spiff / Equipment rebate / Residual-monthly / Auto-Pay residual + Chargebacks / Vendor fee / Misc charges; a tenant adds / renames / deactivates) from ANY carrier's statement | the five column-backed keys in `commcalc.commission_ledger.{commission,spiff,equipment_rebate,residual_monthly,autopay_residual}`; EVERY bucket (incl. the deductions and tenant-defined ones) in (`category`, `payout_total`) — booked from the mapped `raw_amount` by the tenant's own `commission_category_map` rules; a DEDUCTION bucket books the canonical signed amount, so Σ(all active buckets) = the statement's own total | `commission_ledger.classify_line` → `booked_amount` → `build_row` → `summarize`. **Direction first, magnitude second:** `column_mapping.sign_convention` on the amount column says which sign is EARNED, so a chargeback books `−|amt|` into the bucket it reverses and the bucket reads NET. An unmapped label is `'other'` and surfaced, never guessed; a $0 line books nothing (§15, §25.12) |
...
| Activation TYPE of a POS sale line (New activation / Upgrade / BYOD / Port / Hardware only → the buckets premium / upgrade / byod) — every Sales Report / Exec MTD / Daily Targets activation column, every commission activation count, the closing recon's activations / upgrades, the event register's activation lines | `raw_sales` / `daily_sales_feed` lines, ANY column the org's rules name (`accessory_config.activation_details_rules.fields`, default contract_type) × its `tokens` / `exact` map | `line_class.activation_class` → `classify_line` (§3 / §15); DISTINCT `trans_id` per bucket in `_sales_cell_agg` and `calc_rep_commissions`; the intake's 2.5a step maps the words (§30.12) and its save REFUSES a tenant word naming ≥80% of the lines (`refused_tokens`); `GET /exec-mtd/*` → `landing.classified` (the "no line could be told apart" banner, and `rules_refused` — the "every invoice an activation" twin) |
...
| Onboarding intake, the INVOICE kind's verify numbers (2.5 / 2.5b / Stage 4, §30.13): invoices, Σ tie field (net sales by default), Σ invoice total, Σ GP, Σ tax, the tender split by class / by column and Σ tenders vs Σ invoice total per invoice (in words; refused unless attested), Σ tax components; Stage 4: the tender split per store-day beside the X-report (cash / card / other, the difference, "no X-report for this day"), the basis the closing recons read | `raw_sales_invoice` + `raw_sales_invoice_tender` re-read (org × stores × dates × kind); the X-report leg through `closing.router._tender_split_by_store(basis='x_report')`, the invoice leg through `_invoice_tenders_by_store` | `invoice_tenders.invoice_verify` / `tender_tie` / `recon_vs_xreport` → `stage2_refusals(tender_tie=)`; `router._intake_invoice_tender_recon` → `verify_table.tender_note` (`invoice_tenders.stage4_tender_note`) |
| THE TENDER SPLIT PER STORE-DAY every closing cash / card recon reads (Cash Collected, the money recon, the deposit recon, the pickups) — cash / card / other / total per store code | the org's `tender_basis`: `pos_tender_summary` (X-report), `raw_sales_invoice_tender` (invoice), or the X-report else the invoice per store-day; store strings through `_addr_resolver`; the class → gate through `TENDER_RECON_CLASS` | `closing.router._tender_split_by_store` (ONE resolver) = `_xreport_tenders_by_store` — locked (`harness_tender_vocab_lock.py` §e) |
...
**THE KEY IT IS CALLED WITH, for a commission-family statement, is DERIVED — never spelled (2026-09-20, §30.10).**
`commission_ledger.mapping_report_key(statement_type, registry_rows)` is the ONE derivation:
`commission_ledger` for the default type (today's key, byte-for-byte — nothing existing is re-keyed) and
`commission_ledger__<statement token>` for every other statement type (`commission_ledger__residual`), the
token being the report-kind registry's statement-type vocabulary (`report_kinds.statement_type_token`; a third
type is a registry ROW). The router reaches it through `_ledger_mapping_key(client, org, statement_type,
source_report)`; every reader (`_ledger_source_rules`, `_ledger_convention_for`, `_ledger_footer_drop`, the
intake's analyze / commit, `/commission-ledger/analyze` + `/import`, the MA refresh) is keyed by it. The lock
`harness_mapping_key_lock.py` fails the build on a literal `'commission_ledger'` used as a mapping key.
...
of 2026-09-20 and is BUILT in §30.9 — the report-kind registry, mig 1010). Design of record: the workflow architect's `onboarding_flow_design.md` (2026-09-20) — §0 principles, §3
...
### 30.7 THE BUCKETS ARE CONFIG — the registry, deduction buckets, the footer that wore a store, and auto-save (owner 2026-09-20)

Owner, in order: *"need to add the following buckets: Chargebacks, Vendor fee, Misc charges"* → shown a
fixed list of eight: *"again this is not to be hardcoded, the user should be able to define the buckets and
also assign the bucket to a bigger category on the P&L as the others are linked as per the index"* → live at
3.6 on their own statement: *"17 label(s) unassigned … need to assigned buckets, also 1 line for total should
not be appearing as a bucket or a sum of all should also be a bucket"* → *"as you keep assigning the buckets
it should auto save and give an option to save manually also — if you refresh and come out of the module it
is not being saved what you did so far."*

**DUPLICATE CHECK (build gate) — searched §15 (the ledger, `CATEGORIES`, `summarize`), §16 `commission_ledger`
/ `commission_category_map` / `column_mapping` / `report_pull_map` (mig 207, the house-default + tenant-override
pattern), §17 `/commission-ledger/*` / `/commission-category-map`, §18 the bucket and tie-out rows, §4 / the
finance rows of §18 (how commission reaches the P&L), §25.12, §30.1–30.6.** REUSED: the classifier
(`classify_line` / `build_row` / `summarize`), the rules table, the mapping + its convention, the mig-1004
footer rule, the intake spine, `PUT /onboarding/intake/state` (which already merges per key), the P&L chart
`account/coa.PL_SPEC` + `commission_org_config.pl_line_labels`. NEW: one table, three endpoints, one settings
panel — nothing else.

| | |
|---|---|
| **The registry** | `commcalc.commission_bucket` (mig **`1009_commission_bucket_registry.sql`**, written, **NOT applied**): `key` (= `commission_ledger.category`), `label`, `kind` (`earned` \| `deduction`), `sort_order`, `is_active`, `hint_words`, `pl_line_key`, `is_builtin`. House org seeds the five mig-071 buckets + `chargebacks` / `vendor_fee` / `misc_charges` (deduction kind). A tenant INHERITS the house rows and OVERRIDES per key (`commission_ledger.merge_buckets`, the mig-207 pattern) — add, rename, reorder, deactivate, re-hint, re-link. `HOUSE_BUCKETS` in code mirrors the seed (the sign harness parses the SQL and compares) and is what a pre-1009 database reads for DISPLAY |
| **No column per bucket** | The five mig-071 keys stay COLUMN-BACKED and every existing reader of those columns is byte-identical (1,755 line-variants × 3 rule-sets, `harness_commission_ledger_sign.py` §C). Every other bucket is read by (`category`, `payout_total`): `summarize`, `onboarding_intake.bucket_totals`, `/commission-ledger/by-rep`, `_statement_buckets`. `summarize` adds `buckets`, `bucket_source`, `earned_total`, `deductions_total`, `net_total`, `unlisted` (money under a key the registry no longer lists — summed, never dropped); **`payout_total` keeps its meaning: the NET the statement pays** (earned + other + deductions, signed), which is exactly what it summed to before deductions existed |
| **THE SIGN RULE** | canonical = raw × payout_sign (the mapping's convention, §25.12). EARNED bucket: +\|amt\| earned, −\|amt\| netted reversal (unchanged since #246). DEDUCTION bucket: the canonical SIGNED amount whichever way the line points — a −1,500 deactivation books −1,500; a fee written as a positive charge under `payout_negative` books −\|fee\| (the rule targeting a deduction bucket is offered the line; a positive line with NO such rule stays a `charge`, exactly as today — the master-agent feeds' dealer purchases are not re-bucketed); a fee REFUND books positive. Never abs(). **INVARIANT (asserted): Σ over every active bucket = the statement's own total, whatever buckets the tenant chose** — the owner's 521-line statement: reversals in their own `chargebacks` bucket → chargebacks −7,396.27, earned 94,366.61, net 86,970.34; the same file with reversals netted inside the buckets they reverse → chargebacks 0.00, net 86,970.34. **Both are tenant choices; no code default flips silently** |
| **`charge` ≠ `misc_charges`** | `charge` is the NON-payout sentinel (a different money stream on a master-agent feed: books nothing, outside every tie-out). `misc_charges` is a DEDUCTION BUCKET (money the carrier took off the statement, booked signed). `CATEGORY_LABELS['charge']` says so. Not measured here: whether any tenant rule targets `charge` for a label that reads like a deduction — `select org_id, source_report, pattern, category from commcalc.commission_category_map where category in ('charge','exclude')` is the query; changing any is an owner decision |
| **The P&L link** | `pl_line_key` names a line of `account/coa.PL_SPEC` — THE chart every other booking names (validated by `POST /commission-buckets`; the picker lists the chart with the org's `pl_line_labels`). House defaults: commission/spiff → `carrier_comm`, equipment_rebate → `device_rebate`, residual_monthly/autopay_residual → `mi_income`, chargebacks → `chargebacks`, vendor_fee → `vip_fees`, misc_charges → `store_opex` — config rows, an owner/finance decision to change. **RECORDED, NOT BOOKED (the finance boundary, CLAUDE.md):** today NO P&L line reads `commission_ledger`; `coa.build_inputs` books commission from the raw MA tables (§4, §18 "Commission received"), and the ledger's buckets feed only the What-If carrier-income headings (`whatif.LEDGER_INCOME_BUCKETS`). What finance must do to honour the link: read `commission_bucket.pl_line_key` per org in `coa.build_inputs`, book Σ(`payout_total`) per bucket per period to that line (deductions land signed on their expense line), and decide the overlap with the raw-MA bookings (a line booked from both would double) — commission-agent hands over the mapping, touches no P&L code |
| **OPEN — the What-If carrier income** | `whatif._ma_carrier_income` reads the FIVE columns (`LEDGER_INCOME_BUCKETS` commission/spiff/equipment_rebate) + `category='other'`. A line a tenant moves to `chargebacks` leaves the COMMISSION heading (it is no longer netted there) and lands in NO heading: the What-If carrier income OVERSTATES by the deductions from that tenant's next import. Not changed here (it is a displayed figure with its own harnesses); the fix is a `DEDUCTIONS` heading read from (`category`, `payout_total`) for deduction-kind keys — an owner call on presentation |
| **Suggestions are config** | `onboarding_intake.suggest_buckets(labels, …, buckets)` matches every label against every active bucket's `hint_words` (a hint is a STEM on a word boundary: 'upgrade' hits 'Upgrades', 'deact' hits 'Deactivations', 'fee' never hits 'coffee'); the LONGEST hit wins, a tie goes to a deduction-kind bucket, and the sign-mix tiebreak sends a reversal-flagged label (text, or all lines against the earned direction) to its deduction hit — else the FIRST active deduction bucket. Provenance `guess`, never applied. **On the owner's 28-label statement with the house defaults: 27 labels placed, residue NONE** (`harness_onboarding_intake.py` §J) |
| **The footer that wore a store (3.2)** | `split_footer` now has THREE rules: (1) mig-1004 every-identity-blank; (2) the label cell IS the word total / grand total / subtotal — whole cell, never a substring (a carrier or product whose NAME contains the word is never dropped); (3) a row with NO label whose amount equals the sum of EVERY other row to the cent — whatever else it carries. The owner's total row carries the store and the run date; rule 3 catches it. Each footer says its `rule` and `kind`; subtotal rows are footers but never the file's total when a grand total exists |
| **"(blank)" is not a label** | lines with no label that are NOT the footer: `blank_label_issue` → a red banner ("N line(s) carry no label — check the label column at 3.3"), absent from the tray and from `unassigned`, and `commit_refusals` refuses naming 3.3. Never an assignable card |
| **Pre-migration** | `load_buckets_meta` → `ready:false` + the file named; every screen still renders the built-in mirror; `router._ledger_bucket_guard` (in `_ledger_land_rows` AND the MA refresh, BEFORE the slice wipe) and the intake commit REFUSE a landing to a column-less bucket naming 1009 — never re-bucketed; a commit using only the five still lands |
| **Auto-save (design §0.2 / §4)** | `intake-shared.useAutoSave`: ONE debounced (600 ms, trailing) `PUT /onboarding/intake/state` of the step's whole draft (assignments, reversal flags, sign answer, identity decisions, typed total, period, attestation, column map, sheet / header / footer overrides) with the CURRENT step (never advances it); pending patches merge; flushed by the **Save** button on every step (3.1–3.8, 2.1–2.5), on unmount and on `pagehide` / `beforeunload` with a keepalive request. The indicator says **Saving… / Saved ✓ hh:mm / Not saved — why / Not saved: run migration 1007** — a failure is never silent. `PUT /state` merges per key on the backend, so a debounced `assignments` write never clobbers a `sign_answer` written a moment earlier (pinned §K). RESTORE reads every one of those fields back and lands on the saved step |
| **Screens** | intake 3.6 tray (earned / deduction columns from the registry, kind tag, blank-label banner, pre-1009 note) and 3.8 / 3.9 cards (Earned (+) / Reversed (−) / Net, earned and deduction roll-ups, Σ all buckets); the Category → Bucket Map page hosts the **Buckets panel** (add / rename / reorder / deactivate, kind, hint words, P&L line picker, lines-in-ledger) and its rule picker lists the registry; the Commission Ledger tiles (kind-tagged, a "Net payout" tile with earned / deductions, an "unlisted" tile) and the setup wizard render `buckets` / `categories` from the payload. No bucket key is hardcoded in any of them (asserted) |
| **Proof** | `harness_commission_ledger_sign.py` §I–K (registry = seed mirror, merge, the deduction sign rule under all three conventions, the owner's two choices both tie, `unlisted`, ARMED controls: abs()-ing a deduction → RED at net 101,762.88; a deduction that books nothing → RED at 94,366.61; pre-1009 refusal; wiring; RULE TWO) — 125 checks. `harness_onboarding_intake.py` §J–L (the owner's mapping: footer by rule 3, 27 labels, zero residue, earned 94,327.01 + deductions −7,356.67 = 86,970.34, commit + re-read, both choices tie, the three footer rules pure, blank-label banner + refusal, `use_stored=1` re-analyze and commit from the kept file, interleaved `PUT /state` merges, pre-1009 display + refusal, a tenant-defined bucket end to end, the registry endpoints, the page's auto-save / Save / indicator / restore and no hardcoded key) — 164 checks |

**Money surfaced, not moved.** Migration 1009 is written and NOT applied; it seeds house rows only. No ledger
row changes category, no rule changes target, nothing is recomputed. The three deduction buckets take effect for
a tenant only when THEY assign a label to one (a guess is offered, never applied) and the migration has run.
The P&L link is a recorded mapping awaiting finance.
---

### 30.8 STAGE C — the typed "other" reports land in their REAL destinations, bill payments are EXTRACTED from sales, and inventory auto-checks itself against ACTIVATIONS (owner 2026-09-20)

Owner, verbatim: *"other reports land in their respective categories if such a report is present for that
carrier, otherwise bill payments should be extracted from their sales reports and then assigned a separate
report for themselves. merchant payments are the merchant reports which are used to reconcile the credit card
payments received in the store for the transactions vs the one transmitted by the company. X report has their
own report where the data should be uploaded. Inventory also lands on the inventory module where it shows
inventory on hand. inventory report should also auto check itself with the activation report to see which
item has been sold but not rung out properly from the inventory."*

**DUPLICATE CHECK (build gate) — searched §2 (the X-report import `_parse_xreport_detail` and its upsert loop,
`_read_upload_df` / `_read_upload_grids`), §4 / §15 (the mig-939 bill-pay feed resolution `_metric_source` →
`_billpay_processor_name` → `_billpay_processor_by_store_day`; `epay_ingest`; `/upload/ma_daily_tx`), §11 / §11a
(`inventory_aging_device`, `inventory_sold_recon`, `device_cost_recon.device_key`), §12a (`merchant_portals`
normalizers, `merchant_portal_sweep.store_settlement`, mig 955, `closing/external_credit_recon` + the
`report_pull_map` registry row), §23b / §23h (`_addr_resolver`, `_xreport_tenders_by_store`, `_xr_tender_class`),
§3 / mig 962 (`exec_metric_defs.line_match`, the `bill_payment` bucket — the ONE predicate), §6d / mig 313
(`_cr_resolve_activation_details`, `activation_bucketing`), §13a / mig 902 (`storeops.merchant_ids`), §30.6.
NOT rebuilt: no new table, no new parser, no new store map, no second bill-pay predicate.**

| Need | REUSED (not rebuilt) | New |
|------|----------------------|-----|
| The 2.0 "other reports" picker | the Stage-B spine (`source_kind`, instance rows, the shared resolver panel, Stage 4 / 5) | `onboarding_intake.OTHER_KINDS` = `x_report` · `merchant_payments` · `bill_payments` · `other`, `OTHER_KIND_HINTS`, `MATRIX_KINDS` (read by the destination's own parser, no column step), `MATRIX_INSTANCE_SLOT` |
| **X-report** → `pos_tender_summary` | `_parse_xreport_detail` (+ the tenant's `closing_tender_map` labels, `_xr_canon_known`), the upload handler's upsert loop — **factored into `_xreport_land_rows`** (mig-062 natural key, the ONE `_xr_tender_class` rule, upload_log; the upload handler and the intake call the same function — pinned: exactly two `pos_tender_summary` upserts remain in the router, this helper and the flat path); `_intake_store_resolver` + `add_store_alias` for the sheet names, which the closing recon's `_addr_resolver` then resolves through the SAME alias (pinned: `_addr_resolver` and `_xreport_tenders_by_store` read the landed rows) | `_intake_prepare_xreport` (per store-day cash / card / other beside each sheet's own totals row — `xreport_sheet_total`, `xreport_verify`; the close date is the FILENAME's, else typed, never today — `xreport_file_date`, `_XR_NO_DATE` blocks the commit), `_intake_reread_xreport`. An unrecognised tender label is a REFUSAL (its money would be missing from every closing recon), attestable |
| **Merchant settlement** → `merchant_settlement_day` (mig 955) | `merchant_portals.normalize_settlement` (the sweep's own normalizer; `read_table`'s banner rule via `_intake_merchant_table`), `merchant_portal_sweep.store_settlement` (natural-key upsert, `dedupe_settlement`), `merchant_portals.settlement_role` (the source row's override, else the portal's house default, else the person's pick — `external_cc` \| `pos_merchant`, settlement grain ONLY, never funding), `storeops.merchant_ids.resolve_map` / `.upsert` (mig 902 — the SAME map the scheduled pull resolves through; an assign at 2.4 writes it), `closing.router._settlement_rows_for_days` → `external_credit_recon.settlement_cells` reads the landed rows unchanged (pinned) | `_intake_prepare_merchant`: `source_id` = `uuid5(org, portal, role)` — a deterministic UPLOAD source (idempotent re-commit, distinguishable from every pull), `report_key='intake_upload'`; Σ gross / net / fees / count per merchant-day beside the export's TOTAL row (`merchant_totals_row`, `merchant_verify`); **OVERLAP REFUSAL** — a merchant-day another source already landed would be summed twice by the card recon, so the upload is refused naming the days; `_intake_merchant_resolver` (merchant id → mig-902 map, else the daily-tx account index, else the DBA label through the address resolver) |
| **Carrier bill-pay report** → the processor feed the coverage recon reads | the mig-939 resolution (`_intake_billpay_feed` → default layout), `column_mapping` layouts `ma_daily_tx` (existing) + **`epay_daily_tx` (NEW registry entry → `raw_epay_daily_tx`, field spellings = `epay_ingest.COLUMNS`)**, `_ingest_mapped_df` (daily-tx feed: slice replace on account × dates) / `epay_ingest.ingest` (transaction detail: idempotent on transaction id, terminal → store via mig 902), the org's own row predicate `_ma_billpay_pred` (mig 944 + the mig-214 list) / `epay_ingest.is_fee_line`, **RE-READ THROUGH `_billpay_processor_by_store_day` itself** — 'verified' means the coverage recon will read it | `BILLPAY_FEED_TABLES`, `BILLPAY_LAYOUT_FIELDS` / `kind_fields(kind, layout)`, `billpay_feed_verify` (every row, the rows the org's rule counts, Σ per store-day, unmapped accounts), `_intake_reread_billpay`, `router._BILLPAY_FEED_LAYOUT` (the two feed keys the mig-939 reader already dispatches on — the only processor literals, in that one map). Refusals: a layout that is not the feed this org's recon reads; a file in which the org's rule finds no bill payment |
| **Bill payments EXTRACTED from sales** (the derived report) | `exec_metric_defs.line_match` on the org's resolved `bill_payment` rule (`_exec_metric_config`, tenant > house carrier preset > built-in), `_sales_cell_agg`'s three skip rules, `_sales_rows_union`, `_canonical_store_key_fn`, `_billpay_processor_by_store_day` for the carrier side | **`commcalc/billpay_extract.py`** (PURE: `is_countable`, `match_reason` ≡ `line_match` + the token, `extract_lines`, `rollup`, `token_coverage`, `compare_with_feed`); **`GET /commcalc/billpay-extract/{period}`** (`?store=`) and the **Bill Payments** page `commcalc/bill-payments/page.tsx` (NAV Commissions, report directory 'sales'); the sales / POS commit RUNS it over the landed slice (`_intake_billpay_extract_after_sales`) and stamps `verified_numbers.billpay_extract` — the Stage-4 note. **Pinned: the extraction equals `_sales_cell_agg`'s own `bill_qty` / `bill_amt` over the same rows.** The `_BILLPAY_DEFAULT_TOKENS` over-match (§19) is the Daily-Targets vocabulary, not this predicate — nothing widened; tokens that matched nothing are LISTED, edited under Executive MTD → Metric definitions |
| **Inventory → the inventory module** | CONFIRMED: `inventory_aging_device` (migs 216 / 294 / 1004) is what `GET /inventory-sold-recon`, the BS `inventory_basis='devices'`, `/device-history`, `/device-cost-recon` and the payables attribution read (§16) — the Stage-B landing (`b2b_sweep.write_inventory_devices`) IS the inventory-on-hand table; no second table involved | — |
| **Inventory auto-check against ACTIVATIONS** | `inventory_sold_recon.reconcile` (PURE, `device_cost_recon.device_key` injected), the activation feed AS MODELLED — `_cr_resolve_activation_details` (raw_custom_import, one row per device, mig-313 buckets; **now also carries `mdn`** when the export has a mobile-number column, additive) | `inventory_sold_recon`: `activation_index` / `sales_mobile_index` / `sales_evidence` / `mobile_key`, the third finding **`ACTIVATED_NOT_RUNG_OUT`** (on hand + in the activation feed + no sale line, OR sold-then-refunded — `sale_state`), pairing by `device_key` else **`mobile_number_via_sales`** (the number on a sale line, same line or same transaction — stated per row, `via`), everything else UNPAIRABLE with its reason (no key & no number / no sale line carries the number / ambiguous — candidates listed); an activation on a sales hit is EVIDENCE on the one `SOLD_NOT_CLEARED` row, never a second row. `GET /inventory-sold-recon` (`?period=`) returns `activations` + `basis.sources` per side; the page's second section; the inventory commit RUNS it (`_intake_sold_check_after_inventory`) → `verified_numbers.sold_check` → the Stage-4 note "N unit(s) activated but still on hand" |
| **Stage 4 / Stage 5** | `verify_table`, `runbook` | `note` per row (`stage4_note`), `lands_in(kind, payload)` (a bill-pay report records the feed table its layout targets), `runbook.reports` (the two cross-check reports beside the two links, which are unchanged) |
| **Tolerant .xlsx read** (owner blocker, same day: "There is no item named 'xl/sharedStrings.xml' in the archive") | `pd.read_excel` → openpyxl, unchanged, after the ZIP is made to match what it declares | **`commcalc/xlsx_tolerant.py`** (`read_excel(contents, **kw) -> (frame, action)`, `repair`, `describe`, `UnreadableWorkbook`) wired into `_read_upload_df` and `_read_upload_grids` (`router._xlsx_read`); a repair is NOTED (`_note_xlsx_repair` → the upload trace's `xlsx_repair`, the intake's `detect.xlsx_repair`); a non-workbook named .xlsx is refused with a sentence. Proof `harness_xlsx_tolerant.py` (23) — its fixture builds the string table the way Excel writes it (openpyxl 3.1 writes inline strings) |

**THE SAVE GUARANTEE per kind (`_intake_commit_stage2`).** X-report: rows re-read = built = inserted, Σ cash /
card / other / total re-read = shown, the sheets' totals tie (or attested). Merchant: rows re-read = built =
inserted, Σ gross / net / fees AND the txn count re-read = shown, every row carries the stated role, the export's
TOTAL row ties. Bill-pay: the feed table's rows re-read = built = inserted AND the mig-939 reader's Σ over the
file's days = the bill-pay Σ shown (else "the coverage recon's reader does not sum to the bill-pay rows shown —
an unmapped account, or the rule"). Free-text `other`: UNCHANGED — recorded + red.

**Proof: `backend/harness_onboarding_intake_c.py` — 92 checks, DB-free, the Stage-B fake client extended with
the destination tables (declared columns per their migrations), driving the REAL router functions.** §A the
vocabulary and the pure verifies (`match_reason` ≡ `line_match` over a grid); §B the X-report (2 stores, one
unknown → refused → assigned → alias; 8 tender rows; `_addr_resolver` + `_xreport_tenders_by_store` read them;
re-commit upserts); §C the merchant settlement (2 merchants × 2 days, fees, TOTAL; role default and pick;
`store_merchant_id` written + read back; `_settlement_rows_for_days` → `settlement_cells` tallies the landed
days; overlap → refused; bad role / unknown portal → 400); §D the carrier bill-pay report through BOTH layouts
(daily-tx: 24 rows, 12 bill payments by the org's rule, re-read through the mig-939 reader; transaction detail:
the fee line flagged); §E the extraction = `_sales_cell_agg`'s figures exactly, provenance per line, the report
endpoint, the carrier side present → both figures + difference; §F inventory + activations: 2 activated-still-on-
hand (one by IMEI, one by mobile number through a refunded sale's transaction), 3 unpairable with reasons, the
double-count control, the no-feed control; §G Stage 4 / 5; §H the save guarantee per kind, the close-date gate,
the multi-day X-report, the unrecognised tender, the zero-bill-pay file, the wrong-feed layout, the owner's
blocker workbook; §I wiring, registration, RULE TWO (the vendor-name scan now includes the processor vendors),
the pages. Pins CHANGED in `harness_onboarding_intake_b.py` §A (stated): the three typed kinds now have
destinations, each an existing table an existing migration creates. `harness_inventory_sold_recon.py` (43) is
byte-identical green. `harness_xlsx_tolerant.py` (23): its fixture now builds a real string table; F3 states
its intent correctly (a member the stock reader tolerates is read UNTOUCHED, action 'ok').

**Files:** `onboarding_intake.py` (Stage-C vocabulary + verifies), `billpay_extract.py` (NEW, pure),
`inventory_sold_recon.py` (the activation source), `column_mapping.py` (`epay_daily_tx`), `xlsx_tolerant.py`
(NEW, pure), `router.py` (`_xreport_land_rows`, `_intake_prepare_xreport/_merchant`, `_intake_merchant_resolver`,
`_intake_merchant_table`, `_intake_billpay_feed`, `_intake_billpay_row_pred`, `_intake_reread_xreport/_merchant/
_billpay`, `_intake_billpay_extract_after_sales`, `_intake_sold_check_after_inventory`, `_intake_activation_rows`,
`_intake_merchant_portal_options`, `billpay_extract_report`, the `role` form field, `_xlsx_read` +
`_note/_take_xlsx_repair`), frontend `onboarding/intake/stage2.tsx` (the picker, per-kind 2.2 / 2.4 / 2.5 /
2.6), `intake-shared.tsx` (types, the resolver panel's `identityKind`), `page.tsx` (the Stage-4 note, the Stage-5
reports), `commcalc/bill-payments/page.tsx` (NEW), `commcalc/inventory-sold-recon/page.tsx` (the second
section, the per-source basis, evidence), `lib/rbac.ts`. **No new table, no new migration; nothing recomputed.**
...
`_intake_reread_sales`; inventory: `_intake_reread_inventory`; activations: `_intake_activation_rows`;
commission: `_intake_reread`; merchant / X-report / bill-pay: the Stage-C re-reads); exposed on
`GET /onboarding/intake/state` as `report_links` (or one read-only endpoint), a "Report links" section on
Stage 4; the inventory-vs-activation pairing above is its first consumer (`inventory_sold_recon.activation_index`
is the reusable pairing core). No table, no writes, no guessing — an unmatched line is listed as unmatched.
(b) **Layman cards + `detect_report_kind`.** **CLOSED — built in §30.9 (mig 1010 `commcalc.report_kind` + `report_signature`, `report_kinds.detect_report_kind`, the 2.0 cards and the "drop any report here" zone).** The design as it stood: A report-kind registry (label, "what's in it", the signature
columns, where it usually comes from) with house-default copy overridable through the EXISTING
`report_labels` / `ui_label_override` mechanism; pure `detect_report_kind(headers) -> [(kind, confidence,
evidence)]` on `column_mapping.TARGET_FIELDS` aliases (no second alias list); a `report_signature` registry
(mig ≥ 1010, header NAMES only — never a cell value, filename, store, rep or customer string; org copy + house
copy; a fingerprint later confirmed as a different kind → ambiguous → ask) consulted FIRST, keyword heuristics
second; confirm, never silently reassign. Cards: sales with IMEI + phone (`pos`), sales with cost + selling price
...
something else. (d) **§7 availability gating** (`docs/ONBOARDING_FLOW_DESIGN.md` §7): **CLOSED — built in §30.9: ONE registry, ONE visibility function (`report_kinds.visible_kinds` / `carrier-scope.reportKindsVisible`), SIX surfaces routed, the build-failing lock `harness_report_kind_lock.py`.** As it stood: the next PR builds the report-kind registry
...
type"** (not "residual is missing"). **CLOSED — built in §30.10 (the ONE derivation `commission_ledger.mapping_report_key`,
the residual layout `commission_ledger__residual`, the lock `harness_mapping_key_lock.py`, proof `harness_statement_type_mapping.py`).**
As it stood: the next PR fixes it for every statement type at once. Stage A keys `commcalc.column_mapping` by
...
the rules already are (`<carrier_code>__<statement slug>`, e.g. `commission_ledger__residual`) via the ONE writer
`upsert_column_mapping` / `load_rules`; house-default residual TARGET_FIELDS (account / mdn, plan or product
label, residual amount, service month, store, rep); the ledger page filtered by the distinct `source_report`
values; one Stage-4 row per statement type; harness = a residual fixture end to end + two statement types on
one carrier keeping two independent maps and sign answers. Not started here — it is Stage-A money-adjacent.

### 30.9 THE REPORT-KIND REGISTRY — availability is DERIVED from the declaration, never listed; the layman cards; detection that learns (owner directives 2026-09-20; design §7)

Owner, verbatim: *"for the layman we need to be more clear of what report we are trying to get them uploaded — sales
reports with imei and phone number; sales report with cost and selling price; commission report … residual report;
inventory at hand report; inventory aging report; Bill Payment transactions report; X report; any other report which
they feel is needed and the system to check against what report it matches using intelligence gained by using all the
reports."* And: *"it is very important that we don't have extra file upload paths for a new tenant who does not need
those based on the carrier they pick. Our system should be smart enough to only show those which are carrier-specific
once they have been defined by a previous tenant or us on the back end … no patchwork, it should work as a design.
Currently in the Verizon tenant we have all the table uploads for B2B when it has been declared that the POS is not
B2B, it is RQ."*

**THE CLASS (measured before building — CLAUDE.md "A fix is a DESIGN fix").** *Upload surfaces do not dereference the
tenant's declaration.* SIX surfaces offer uploads — `commcalc/upload/page.tsx` (gated only its POS email-report block,
by a POS vendor name hardcoded as `CUSTOM_REPORTS_POS`, RULE TWO failing), `commcalc/upload/wizard/page.tsx` (a
vendor-named `FALLBACK_STEPS`), `commcalc/email-imports/page.tsx` (`BUILTIN_TYPES`, `DEFAULT_RULES`, a `suggestRule`
pattern list, the "Apply b2bsoft standard" preset — the owner's screenshot on the Verizon tenant), `commcalc/ftp-
imports/page.tsx` (the same mechanism; found by the lock's first run, not by the measurement), the intake's 2.0 cards
(`onboarding/intake/stage2.tsx` + `intake-shared.tsx`) and the #234 tile block. Live instance: org `f4f1c16e…` has
`ui_label_override` scope `report_term` key `pos_system` = 'RQ' and still saw every B2B path on five of them.

**DUPLICATE CHECK (build gate) — searched §16 (`report_pull_map` 207, `pos_profile` 200, `report_definitions`
039/291, `ui_label_override` 068/945/953, `column_mapping` 042, `onboarding_state` 927, `commission_bucket` 1009), §17
(`/upload-registry`, `/connectors`, `/report-labels`, `/nav-config`, `/nav-labels`, `/pos-profiles/*`,
`/custom-import-types`), §26 (`carrier_visible`, `upload_scope_map`), §30.8 OPEN (b)/(d), PR #234 (`posVisible` /
`posOK` / `POS_GATED_SURFACES`, the super-admin `cap` override).**

| Need | REUSED (not rebuilt) | New |
|------|----------------------|-----|
| The tenant's declaration | the `pos_system` vocabulary term (mig 953; `report_labels.load_report_labels` → `term_from_payload`, tenant override > carrier preset), the org's active `pos_profile` rows (mig 200 — applying a POS standard is a declaration), the org's `commcalc.carrier` rows through `normalize_carrier_code` | **`report_kinds.tenant_declaration`** — THE ONE READER (pure core `declaration_from`); unknown → `reasons`, and unknown HIDES the specific kinds (show less, say why) |
| What a kind applies to, who defined it, its layman copy | the mig-207 house-default + tenant-override-per-key shape; `column_mapping.TARGET_FIELDS` names as `signature_fields` (no second alias list) | **mig `1010_report_kind_registry.sql` — `commcalc.report_kind`** (`org_id`, `key`, `label`, `what_in_it`, `recognisable_columns[]`, `source_hint`, `applies_to_pos[]`, `applies_to_carrier[]` — CODES as data, `{}` = any; `defined_by` house\|tenant + `defined_by_org`; `statement_type` commission\|residual; `landing` = the intake source kind or `custom_import` \| `carrier_report` \| `module`; `layout`; `signature_fields[]`; `requires_columns[]` / `excludes_columns[]` — on-hand vs aging decided by a received / age column; `upload_types[]` = the legacy route keys / tile ids; `custom_sheet_label`; `sort_order`; `is_active`). House seed = the eleven layman cards + every report the Upload page / wizard offered by hand (their carrier / POS scope now DATA); **`report_kinds.HOUSE_KINDS` is the byte-equal code mirror** (the seed is generated from it; `harness_report_kinds.py` §A parses the SQL back) — `registry_ready:false` until applied |
| Visibility, everywhere | the `carrier_visible` / `posVisible` family; the super-admin `cap` override (`ui_label_override` scope 'cap', `POST /nav-labels`, `/admin/labels`) — **`kind:<key>` is one more namespace** in it (widening = super-admin only, the same asymmetry) | **`report_kinds.visible_kinds`** (backend) ≡ **`carrier-scope.reportKindsVisible`** (frontend), clause for clause: active ∧ (POS ∩ declared ∨ any) ∧ (carrier ∩ declared ∨ any), hide override wins, show override = 'widened by super-admin'; provenance per row (house default / your override / defined by a tenant on this carrier / confirmed by you N times); `hidden_kinds` says WHY per withheld row; `for_surface` per surface; `upload_types_of`; `filename_rules_for` (the declared POS's `pos_profile.filename_rules`, house row inherited — **`_pos_profile` now falls back to the HOUSE row for the same `pos_key` and answers None for an undeclared POS, never another POS's rules; `POST /pos-profiles/{pos}/apply` refuses (400) for a POS with no standard**) |
| The payload every surface renders | — | **`GET /commcalc/report-kinds`** → `report_kinds.payload`: `declaration` (+ `reasons`), `kinds` (visible, with provenance), `hidden` (with why), `surfaces` (keys per surface), `upload_types`, `filename_rules`, `standard` (the declared POS whose standard the preset applies), `caps`, `all_keys`, `registry_ready`; org-scoped `{org, house}` on every read. Frontend: **`lib/report-kinds.ts::useReportKinds`** — the ONE hook, the ONE caller of `reportKindsVisible`, the ONE fetch; nothing is offered until it loads and a failed read says so (never a wider fallback) |
...
hotsheet, daily closing) — NO payment detail / MI & ATU / comp report / marketplace feeds / distributor workbook /
asset ledger / inventory-recon link, and NO POS email-report block (Activation Details, Bill Payments, Sales by Product
are B2B rows); the "what decides this" line names the declaration and the 18 withheld kinds. Wizard: only routes a
visible kind names. Email + FTP imports: "Routes to" without the marketplace / processor types; no filename standard
(none is defined for POS `rq` → the button is replaced by "no filename standard defined for RQ yet"), a new mailbox
starts with NO rules and says so. Intake 2.0: the ten POS-agnostic cards. Everything withheld is listed with why.
The same tenant BEFORE the migration sees the same set from the mirror, with `registry_ready:false` stated.

**Proof.** `backend/harness_report_kinds.py` (115 checks, DB-free, an in-memory client driving the REAL router:
§A mirror = seed; §B the reader over the live shape of org `f4f1c16e…`; §C the Verizon case on all five surfaces +
siblings + widening + unknown → less; §D projections / rules; §E detection fixtures — line-level sales with IMEI +
phone, sales-by-product `Invoice # / Product SKU / Tracking # / Sold By / Total Price / Total Cost / Gross Profit`,
commission `Gross / Report Section / Report SubSection / AgentSSOID / Master Service Date`, residual, inventory
on-hand vs aging, X-report, merchant, bill-pay, ambiguous → ask, nonsense → none; §F learning with tainted fixtures;
§G `GET /report-kinds` / `POST /report-kinds/detect` / the learn hook / `_pos_profile`; §H registration + RULE TWO).
`frontend/prove_report_kinds.mjs` (45, the REAL transpiled `carrier-scope.ts` + `report-kinds.ts` + `uploadRoutes.ts`
over the seed parsed from the migration — the page-level selectors). `harness_report_kind_lock.py` (20, incl. the
nine negative controls). Unchanged and green: intake 164 / B 87 / C 92, carrier-scope 64, the three CI guards.

**Files.** `report_kinds.py` (NEW, pure + thin loaders), `router.py` (`/report-kinds`, `/report-kinds/detect`,
`_intake_caps`, `_intake_learn_signature`, the `report_kind` form field on `/onboarding/intake/commit`, `_pos_profile`
ladder, `nav-labels` `kind:` namespace), mig `1010`, frontend `lib/carrier-scope.ts` (`reportKindsVisible` +
types), `lib/report-kinds.ts` (NEW), `commcalc/_lib/uploadRoutes.ts` (NEW), the six surfaces, `admin/labels`.
**Nothing recomputed, no money row touched, migration NOT applied.**

**Seams left (reported, not hidden).** (1) `commcalc/implementation/page.tsx` lists the connector registry's reports
and names the house POS in a how-to — excused in the lock with its reason; routing it through `/report-kinds` is its
own PR. (2) The closing module's X-report attach (`DailyClosingVerify`) and tender-source picker are excused with
reasons (x_report is POS-agnostic, so the registry answers "shown" for every tenant today). (3) `AUTO_SOURCES` on the
Upload page (the portal sweeps) keep their connector-registry carrier scope — they are automations, not upload
choices. (4) A filename standard for a POS other than the house one is a `pos_profile` row nobody has written yet: the
Verizon tenant is told so instead of being handed another POS's rules.

### 30.10 THE COLUMN-MAPPING KEY IS DERIVED PER STATEMENT TYPE — the residual mapping, and the class it belonged to (owner 2026-09-20)

Owner: *"residual report … also add the residual mapping."* House rule: fix the mechanism for every caller, find the
siblings, one fact one home, lock it.

**THE CLASS (confirmed in the PR #254 review, §30.8 OPEN (c)) — "a column-mapping key that ignores the statement type",
not "residual is missing".** Stage A keyed `commcalc.column_mapping` for the intake by (org, `commission_ledger`,
carrier_id, target_field) — `_intake_commit_commission` through the module constant `_INTAKE_REPORT_KEY`; the older
wizard, `_ledger_source_rules`, `_ledger_footer_drop`, `_ledger_convention_for` and the MA refresh spelled the same
literal. A second statement type from the SAME carrier (the residual statement — a different layout, its own sign)
would therefore have OVERWRITTEN the commission statement's column map AND its `sign_convention` (mig 1006 lives on
that key's `raw_amount` row). The RULES were already namespaced per statement (`commission_category_map.source_report
= <carrier>__<statement slug>`, §30); the mapping was the one fact without its home. Measured before building:
`harness_statement_type_mapping.py` §F re-keys through the literal and shows the residual commit rewriting the
commission statement's amount header to `Residual` and its sign to `payout_negative_netted` — the pin goes RED.

**DUPLICATE CHECK (build gate) — searched §16 `column_mapping` (042/1006/1008), §17 `/column-mapping*`,
`/commission-ledger/analyze|import|summary|templates`, `/onboarding/intake/*`, §25.11–25.12, §30.1–30.9 (the registry's
`statement_type`, `kind_key_for`, `source_report_key`), the design's §4 `source_mapping` (one row per (org, carrier,
statement type) — the INTENT, now met by the key rather than a new table). NOT rebuilt: no new table, no second writer,
no second alias list, no second footer rule.**

| Need | REUSED (not rebuilt) | New |
|------|----------------------|-----|
| The key | the ONE writer `upsert_column_mapping` (§25.11) and `column_mapping.load_rules` — behaviour unchanged, called with the derived key | **`commission_ledger.mapping_report_key(statement_type, registry_rows)`** — THE derivation: `commission_ledger` for the default type (today's key, byte-for-byte), `commission_ledger__<token>` otherwise; `mapping_report_keys` (one per registry statement type, for the picker); `statement_type_of_source_report` (a read endpoint recovers the type from `<carrier>__<slug>`); generic mechanism `column_mapping.variant_report_key` / `split_report_key` (`REPORT_KEY_VARIANT_SEP = '__'`) |
| The vocabulary | the report-kind registry's commission-family rows and their `statement_type` (mig 1010; `HOUSE_KINDS` before it — §30.9) | **`report_kinds.statement_types(rows)`** (mirror ∪ rows, default first — a key never changes because a row was edited) and **`statement_type_token(text, rows)`** ('residual statement' → `residual`; blank / 'commission statement' → the default; a text naming NO known token keys as its own slug, never onto the default's key); `kind_key_for` now resolves through it (no type named in code); `STATEMENT_TYPE_DEFAULT` |
| The residual layout | `column_mapping.TARGET_FIELDS` + `TABLE_MAP` (the layout registry), the alias mechanism `suggest`, the footer rule through `identity_fields` | **`TARGET_FIELDS['commission_ledger__residual']`**: line identity (account / mobile number → `account_id`, generic aliases incl. MDN / Mobile Number / CTN), account name, store, rep, order id, residual type, plan or product (`product_name`, required), service month (`trans_date`, `date_auto`), residual amount (`raw_amount`, number, required — the 3.4 sign question applies per type). Lands in the SAME ledger table. `_base_fields`: a variant key with no entry inherits the base key's fields, so a third type maps with no code |
| The registry card | the residual card of §30.9 | its `layout` is now `commission_ledger__residual` with `signature_fields` through it (mirror + 1010 seed, pinned equal by `harness_report_kinds` §A; 1010 NOT applied) |
| The I/O | `_report_kinds.load_registry` (rows, else the mirror) | **`router._ledger_mapping_key(client, org, statement_type, source_report)`** — the wrapper every endpoint calls; `_ledger_carrier_of_source_report` resolves the CARRIER of an intake-written `source_report` through `implementation_spine.carrier_id_by_code` so the read endpoints read the carrier-scoped rows |
| The callers routed | `_intake_prepare_commission` (derives once → `ctx['report_key']`, the analyze payload's `report_key`), `_intake_house_defaults` (house rows under the SAME type's key), `_intake_commit_commission` (saves and reads back under it; the layout learned under it), `/commission-ledger/analyze` + `/import` (new optional `statement_type` Form; else the type off the template's `source_report`; payload `report_key`), `_ledger_source_rules` / `_ledger_footer_drop` (`report_key=`), `_ledger_convention_for(..., source_report=)` for `/commission-ledger/summary`, `/observed-types`, `/commission-category-map`, `/ma-class-wiring/ledger-delta`, `_ledger_ma_derive` (the MA refresh = the default type, derived), `onboarding_intake.REPORT_KEY_BY_KIND`; `column_mapping.known_report_keys` lists one key per registry statement type (the Column Mapping picker) | `_INTAKE_REPORT_KEY` DELETED (the constant was the defect) |
| Frontend | `useReportKinds` (the one hook, §30.9) on both pages | `lib/statement-type.ts::statementTypeToken` (the token's twin, so the highlighted card and the saved key agree); intake 3.1 offers `<token> statement` per registry row (no type named); **Commission Ledger — Setup** offers the registry's statement types, sends `statement_type`, and saves its column choices under the `report_key` the analyze payload names (it spelled `'commission_ledger'` before — a sibling); `carrier-scope.ts` `statement_type` widened to `string` |
| THE LOCK | the carrier-vocab guard's posture (stdlib static scan, one CI job — `carrier-vocab-guard.yml`) | **`backend/harness_mapping_key_lock.py`**: (a) a literal `'commission_ledger'` on a line that reads or writes the mapping, anywhere in backend/app, the harnesses, the scratchpad or frontend/src, outside an ALLOW entry with its reason (stale entry fails); (b) the old constant, or `MAPPING_REPORT_KEY` referenced outside the deriving module; (c) the derivation defined once, the wrapper present, the setup page saving under the payload's key; (d) nine negative controls |

**What is unchanged for every existing tenant: nothing moves.** The default statement type derives TODAY's key, so every
`column_mapping` row and every sign answer saved before this (§25.11–25.12, Stage A) is read exactly as before — proven
by §C of the harness (rows seeded under the pre-change key load byte-identically through `_ledger_source_rules` with no
key passed, through the intake with no type stated, and a commit UPDATES those rows in place). The ledger's
`source_report` (`<carrier>__<slug>`) and the rule namespace are untouched. No migration; no row rewritten.

**What a residual upload now does (the Verizon tenant, or any).** 3.1 offers "Residual report from the carrier" from the
registry; the intake derives `commission_ledger__residual`, proposes the residual layout's columns (amount ← Residual,
label ← Plan, line ← Account / MDN, period ← Service Month) with NO prefill from the commission statement's map, asks
3.4 afresh for THIS type, buckets the labels through the mig-1009 hint words (`residual`, the auto-pay spellings — already
present, nothing added), lands under `verizon__residual_statement`, ties out to its own total, and Stage 4 shows one row
per (carrier, statement type). The commission statement's map and sign answer are byte-identical afterwards. The
Commission Ledger page's template picker lists both `source_report` values from the data.

**Proof:** `backend/harness_statement_type_mapping.py` — **62 checks**, DB-free, the intake fake-client pattern over the
REAL router: §A the derivation; §B one carrier, two statement types → two independent maps, two independent sign answers
(`payout_positive` / `payout_negative_netted`), each landing and tying to its own total, Stage 4 one row per type, the
read endpoints reporting each type's own convention; §C the migration-free compatibility pin; §D the older wizard reads
the same key (incl. the type read off the template's `source_report`, and the untouched `ma_daily_tx` path); §E a third
statement type as a registry row only (its own key, fields inherited, learned under its card); §F negative controls
(re-key through the literal → RED; the lock scanner red on a literal); §G wiring, RULE TWO, registration.
`harness_mapping_key_lock.py` — 17 checks. Unchanged and green: intake 164 / B 87 / C 92, sign 125, mapping_save 15,
report_kinds 119, report-kind lock 20, ma_sync 113, prove_report_kinds 45, the three CI guards.

**Seams left (reported, not hidden).** (1) mig 1010's `report_kind.statement_type` CHECK admits `commission | residual`
only: a third statement type's registry row needs that CHECK widened — a migration, not code (the harness proves the
code path; the database would refuse the row today). (2) The ledger has no mobile-number column, so a residual line's
MDN maps to `account_id` (the line identity); linking a reversal to its original line by MDN (design §3.6) needs a
column — an owner decision. (3) A statement type typed at 3.1 that names no registry token keys as its own slug
(`commission_ledger__spiff_file`); if that token is later added as a registry row the key changes to
`commission_ledger__spiff` and the earlier map must be re-confirmed — the intake shows it as unmapped rather than
...
### 30.12 STEP 2.5a — WHAT COUNTS AS AN ACTIVATION: the words are mapped at intake, one predicate counts them everywhere (owner 2026-09-21)

Owner, verbatim: *"sales report shows 88 txns but not a break up in to activations and upgrade etc, also nothing on
exec mtd"*.

**Measured (org f4f1c16e…, August 2026):** 1,056 line-level rows, 88 distinct invoices, `contract_type` BLANK on every
row (the export has no such column). The activation TYPE is in the category PATH leaf — `… >> New Activation (…)` (40
lines), `… >> Upgrades (…)` (25), `… >> Cellular Equipment >> Customer Provided Device` (24; product "Customer Owned
Device …"), `… >> Prepaid/Hardware Only (Equip)` (4) — and the handsets under `… >> SmartPhones >> …` (61) / `… >> Basic
Phones >> …` (6). `_exec_mtd` read 0 for every activation column and 0 phones (`metric_coverage` flagged `phones`);
the Sales Report showed 88 transactions with no split. No tenant rows in `exec_metric_config`, no
`activation_details_rules` of its own.

**THE CLASS.** A sale line's activation type was classified from ONE fixed column (`contract_type`) by token lists
hard-coded in three places (`calculator.classify_contract_type`, `router._exec_act_class`, the mig-213
`_resolve_ct_bucket`), and the Exec-MTD line buckets from a house vocabulary. A POS that carries the same fact in
another field — or in other words — yielded silent zeros on every report and in every commission calculation.

**THE FIX (config, one home, mapped at onboarding, locked).**
1. **One predicate.** `commcalc/line_class.py` — `activation_class(row, rules)` /
   `classify_line(row, rules)` (§3). Rules per org in the EXISTING `accessory_config.activation_details_rules`
   JSON (mig 313, extended with `fields` / `tokens` / `exact` — no migration): which COLUMNS carry the type and which
   WORDS name each class, matched by contains over every configured column; the mig-213 `contract_type_map` is the
   `exact` layer of the same fact. House defaults = the retired chain, byte-identical (`harness_line_class.py` §B).
   Every sibling dereferences it (the list in §15's row); `activation_bucketing` (the carrier-portal Activation-Details
   basis), `asset/router._promo_type` (the hotsheet promo taxonomy) and `sale_installment_engine` (the residual
   category chain) are excused BY NAME in the lock, each with the reason it is a different question.
2. **The Exec-MTD line buckets** stay `exec_metric_config` (mig 204/962/963); `exec_metric_defs.line_match` gains
   `category_contains` / `department_contains` (additive) so a category PATH can be matched; `metric_coverage` stays
   the detector; the retired `activation` bucket row is no longer offered by the Metric-definitions panel.
3. **The intake maps it, with intelligence, and the person confirms — step 2.5a** (`onboarding_intake.STEPS_STAGE2`,
   between 2.5 and 2.6; `frontend …/onboarding/intake/line-class-step.tsx`, rendered by `stage2.tsx`): the
   suggestion engine (`line_class.suggest_rules` / `suggest_metric_rules`) scans the DISTINCT values of
   `contract_type` / `category` / `department` / `product_desc` / `trans_type` in the frame (analyze) or the LANDED
   slice (`GET /onboarding/intake/line-class`), matches generic hint words per class (`HOUSE_HINTS` /
   `HOUSE_METRIC_HINTS`, overridable per org under `hints` / `metric_hints` in the same JSON), drops a word that
   names ≥80% of all lines (a department word inside every category path — "activation" here — is TOO BROAD and is
   reported, never proposed), shows each proposal's column, sample values and the count it would classify — computed
   by THE predicate, so what is confirmed is what every report counts. Over the measured vocabulary it proposes
   category "new activation" → activation, "upgrade" → upgrade, category "customer provided" + product "customer
   owned" → BYOD, "hardware only" → hardware_only, and `category_contains` "smartphone" / "basic phone" → phones.
   Save = `PUT /onboarding/intake/line-class` → `put_accessory_config(activation_details_rules)` +
   `put_exec_metric_config` (the two writers; nothing else) → the landed slice RE-COUNTED onto the stage row.
4. **The gate.** A line-level sales commit whose landed slice classifies ZERO activation-type lines LANDS (the rows
   are in the table) but is NOT verified: `onboarding_intake.activation_gate` → stage row `needs_input`, blocking
   reason naming step 2.5a, Stage-4 `fix_step` = 2.5a and `activation_note`; verified once a rule classifies ≥1 line
   or the person attests "this file truly has no activations" (`attestation.no_activations` on the commit form, or
   `no_activations` on the 2.5a save — recorded with the name in `verified_numbers.activation_gate.attested`). The
   Exec MTD payload's `landing.classified` (the same `count_classes` over the page's own rows) drives the banner
   *"… no line could be told apart as an activation or upgrade — map it under Onboarding — Commission Intake (step
   2.5a)"* (ScreenLink; no second link map).
5. **Lock** (`backend/harness_line_class_lock.py`, in `carrier-vocab-guard.yml` beside the other locks): no second
   token list / retired classifier / contract-type token check outside `line_class.py` (excused files named, each
   re-verified); every caller dereferences (`classify_line` / `_lc.activation_class`), no app module calls the bare
   alias; `activation_details_rules` touched only by the loader, the writer and the two resolvers; the 2.5a step
   touches no table of its own; the surfaces render the step, the banner and no retired editor row; negative
   controls prove each rule can go red.

**Counting rule, stated:** activations are DISTINCT invoices per class (`_sales_cell_agg`, `calc_rep_commissions`);
an invoice with two new-activation lines counts once; an invoice carrying two classes counts in both
(`activation_bucketing.mixed_bucket_transactions` measures that). Over the measured-shape fixture — 40 / 25 / 24 / 4
lines on 88 invoices — the split is **39 activations, 25 upgrades, 23 BYOD** (Total Activation 87; the hardware-only
invoice is not one), Total Phones 67; on the live tenant the same rule yields its own distinct-invoice counts.

**Money.** The pay path (`calc_rep_commissions` via `cfg['line_class_rules']`, `payout_accrual`, the plan engine)
now reads the SAME rules the reports read. Under house defaults every existing tenant is byte-identical (the pin
replays the retired classifiers over 1,554 spellings × empty / foreign / pinned maps + 4,000 fuzzed labels). A tenant
with a `contract_type_map` and a legacy-engine carrier mode would now also PAY on its mapped labels (the plan
engine already did); no such tenant is known — stated, not hidden. A tenant-authored exec `activation` byod /
upgrade token wider than the built-in word used to only suppress the Port sub-split and now classifies the line —
measured at 0 spellings on the battery. Migrations: **none**.

**Duplicate check (what was searched, what is REUSED):** `activation_details_rules` (mig 313 — extended, the same
column), `contract_type_map` (absorbed as `exact`), the mig-224 `activation_rules` (transaction-level, kept as the
supplement it is), `exec_metric_config` + `exec_metric_defs.line_match` (extended with two contains keys),
`metric_coverage` (unchanged detector), `put_accessory_config` / `put_exec_metric_config` (the two writers),
the intake auto-save + `_intake_save_state`, `_intake_reread_sales` (the slice re-read), `ScreenLink`
(`onboarding_intake`), `landing_identity` (`landing` payload extended with `classified`). The fake supabase client
is factored out of `harness_onboarding_intake_c.py` into `backend/harness_intake_fakes.py` (with the measured
fixture) so the new proofs carry no fourth copy. Proofs: `harness_line_class.py` (35), `harness_line_class_lock.py`
(15), `harness_onboarding_intake_d.py` (34); `harness_onboarding_intake.py` pins the 2.5a Save row.

**Files:** `backend/app/modules/commcalc/line_class.py` (new); `calculator.py`, `router.py` (`_accessory_config_uncached`,
`_line_rules_of`, `_line_skip`, `_blank_ct_bucket_map`, `_classification_gaps`, `_sales_cell_agg`, the calc endpoint,
`_exec_mtd` landing, `/sales-report/detail`, `_intake_line_class_block`, `_intake_landed_slice`, `_intake_prepare_stage2`,
`_intake_commit_stage2`, `onboarding_intake_line_class` / `_put_line_class`, `put_accessory_config`), `onboarding_intake.py`
(`STEP_LINE_CLASS`, `activation_gate`, `activation_note`, `verify_table`), `exec_metric_defs.py`, `commission_engine.py`,
`plan_options.py`, `whatif.py`, `sales_comparison.py`, `payout_accrual.py`, `closing/router.py`, `marketing/event_sales.py`
+ `marketing/router.py`; frontend `onboarding/intake/line-class-step.tsx` (new), `stage2.tsx`, `page.tsx`,
`intake-shared.tsx`, `commcalc/exec/mtd/page.tsx`; `.github/workflows/carrier-vocab-guard.yml`;
`docs/ONBOARDING_FLOW_DESIGN.md` §10.

**ADDENDUM 2026-09-21 — THE SECOND CLASS: "the effective rule is not what the person confirmed".** The owner opened
2.5a on the landed August export and saved; the database then held `activation_details_rules = {fields: ['category'],
tokens: {activation: ['new activation', 'activation', 'add a line', 'new act', 'prepaid'], upgrade: ['upgrade'], byod: ['customer
provided', 'byod', 'customer owned'], port: ['port'], hardware_only: ['hardware only']}}` and **0** tenant rows in
`exec_metric_config` (the phones tick did not persist). `_exec_mtd(…, "August 2026")` → activation **88** / upgrade 9 / byod 15 /
total_phones **0**; `landing.classified` activation **856 of 898** scanned lines → every invoice an activation, because the bare word
"activation" sits inside every category path (`Activations (Price Sheet) >> …`). With `tokens.activation = ['new activation']` only,
the same rows count 25 / 17 / 15 / 2 by distinct invoice. Four faces, one mechanism, each fixed for every caller:

| # | Face | The fix (one home) |
|---|------|--------------------|
| 1 | **The too-broad guard ran over the PROPOSALS, never over what is SAVED.** | `line_class.token_shares` / `broad_tokens` / `refused_tokens` measure the EFFECTIVE rules over the rows in hand; `PUT /onboarding/intake/line-class` resolves the merged JSON through **`router._line_rules_resolve`** (the loader's own resolver, factored out — no second resolution) and REFUSES (400, nothing written, each word named with its share and class) unless `broad_ok` attests the word by name — recorded in the same JSON under `broad_attested['<class>:<word>'] = {by, at, ratio}` (pruned when the word leaves the rule). Basis: the LANDED slice, or the kept file's frame re-read through THE prepare path (`_intake_draft_frame` → `_intake_prepare`) when nothing is landed; "not measured" is stated when neither exists and the commit re-validates. Everything is validated first; the metric rows are written before the activation rules so a failed write leaves nothing half-saved. **Re-validation of what is already saved:** `count_classes` carries `broad` / `refused` (tenant-declared tokens only — house defaults are never measured, the pin); `onboarding_intake.activation_gate` is BLOCKED by a refused word (no "no activations" attestation closes it), `activation_note` reads *"rule refused — "activation" names 95% of 898 lines — fix the words at step 2.5a"* (never "activations: 88 new activation"); the GET block carries `refused` / `rules_ok` / `refusal_note` and the step opens in the refusal state; Exec MTD's `landing.classified.rules_refused` + note drive a red banner beside the silent-zero one. The guard's ratio is `BROAD_RATIO` (≥ 0.8), the same the hints use. |
| 2 | **House contract-type tokens leaked across a field switch.** `resolve_rules` filled every undeclared class from `HOUSE_TOKENS` (" aal", "idv", "port" …); under `fields: ['category']` they applied to the wrong column. | **THE NO-LEAK RULE** in `resolve_rules`: `house_fill = fields == HOUSE_FIELDS`; the house tokens, the legacy exec `activation` layer and the mig-213 auto tokens fill an undeclared class ONLY under the house field; under tenant-declared fields an undeclared class has `[]` (the step shows it empty and says why). A tenant with declared tokens but house fields keeps today's behaviour; every org with no declaration is byte-identical (`harness_line_class` §B pin + G8). `merge_into_raw` now stores ONLY the classes given (per-class merge; no copy of the house list in a tenant row — the resolver fills) and prunes attestations. |
| 3 | **The editable words were seeded from the wrong source; hint words too generic on their own.** | The step's editable words come from **`suggest_rules(...).proposal` only** (`frontend …/intake/line-class-logic.ts` `seedFromBlock` — a pure module the step imports; the block the router returns carries `rules: {fields, tokens, exact, source, declared, house_fill}`, never a hint list). The proposal per class = the person's declared words MINUS any refused, PLUS the file's hint hits; with nothing declared, the hits — else the house words under the house field only, `[]` under tenant fields. **Decision on `HOUSE_HINTS`:** the bare words `activation` and `port` are REMOVED as hints (a department word inside every path / a substring of "support"); the guard would catch them only once proposed and only at ≥ 80% — a word wrongly naming 30% of the lines would sail through — so a word generic on its own is never a proposal. `port` hints are now `port-in`, `port in`, `port in activation`, `number port`, `ported`. |
| 4 | **The metric tick did not round-trip.** ROOT CAUSE = **server**: mig 962 DROPPED mig 204's `UNIQUE (org_id, bucket)` and created the unique index `(org_id, bucket, carrier) NULLS NOT DISTINCT`; `put_exec_metric_config` still upserted `on_conflict='org_id,bucket'` → Postgres 42P10 ("no unique or exclusion constraint matching the ON CONFLICT specification") → `{ok: false, hint: "run migration 204"}` → the 2.5a PUT raised 400 AFTER `put_accessory_config` had already written (the owner's row persisted, the phones row did not). Every `PUT /exec-metric-config` (the Metric-definitions panel too) has failed on a post-962 database. | `put_exec_metric_config` upserts `{…, carrier: None}` with `on_conflict='org_id,bucket,carrier'` and falls back to the legacy target on a pre-962 database (the reader's own column ladder); the error is reported verbatim, never as "run migration 204". `harness_intake_fakes.FakeDB` gains `unique_indexes` (a conflict target must name one → 42P10, as Postgres) so the fake REPRODUCES the defect (`harness_onboarding_intake_d` H5) and proves the fix (H6–H8: the row written, updated in place, pre-962 shape still served, `_exec_mtd` `total_phones` 67 > 0). The frontend proof (`prove_line_class_step.mjs`, the real transpiled TS over the engine's own block) pins that ticking "use" puts `metric_rules.phones` = the proposal in the body. |

**Money.** The classifier feeds pay: under house defaults every existing tenant is byte-identical (the §B pin unchanged: 1,554 spellings + 4,000 fuzzed labels; G8 / H15 add "nothing refused, house_fill True"). A tenant with tenant-declared fields and an undeclared class no longer classifies that class by a contract-type word (the no-leak rule) — the only behavioural delta, and it is the defect. Migrations: **none**.

**Lock** (`harness_line_class_lock.py`, now 29): (f) the save resolves through `_line_rules_resolve`, measures (`_lc.refused_tokens(`) and refuses BEFORE either writer (a hoisted write → RED; a second `_lc.resolve_rules(` in router.py → RED); the block hands no hint list (`"rules": rules` → RED); the step seeds through `seedFromBlock` reading `suggest.proposal` only (a seed from `block.rules` → RED; a step reading `hints` → RED); behavioural: a saved broad word → refused, the attested word → not refused, tenant fields + undeclared class → no house / legacy / auto token, the pin; the bare hints absent; the frontend proof exists.

**Proofs:** `harness_line_class.py` 35 → **47** (§G), `harness_onboarding_intake_d.py` 34 → **49** (§H: the owner's state seeded verbatim → refused on GET, Stage-4 "rule refused", the save refused with nothing written, the root cause reproduced, the correction verified 39 / 25 / 23 + 67 phones, the attestation by name, the pre-landing frame basis, the house pin), `harness_line_class_lock.py` 15 → **29**, `frontend/prove_line_class_step.mjs` **17** (new).

**Files:** `line_class.py` (`token_shares`, `broad_tokens`, `refused_tokens`, `rules_refused`, `refusal_sentence`, `norm_broad_ok`, `attest_key`, `count_classes.broad/refused`, `resolve_rules.house_fill`, `merge_into_raw` per-class + attestations, `HOUSE_HINTS`), `router.py` (`_line_rules_resolve`, `_intake_draft_frame`, `_intake_line_class_block`, `onboarding_intake_put_line_class`, `put_exec_metric_config`, `_exec_mtd` landing), `onboarding_intake.py` (`activation_gate`, `activation_note`), `harness_intake_fakes.py` (`unique_indexes`); frontend `intake/line-class-logic.ts` (new), `intake/line-class-step.tsx`, `lib/client.ts` (`e.detail`, additive), `commcalc/exec/mtd/page.tsx`; `docs/ONBOARDING_FLOW_DESIGN.md` §10.

---

### 30.13 SALES BY INVOICE WITH THE TENDER TYPES — a kind of its own, the tender columns DECLARED at step 2.5b, ONE tender vocabulary, and the cash-collected basis as config (owner 2026-09-21)

Owner, verbatim: *"sales by invoice report also has the tender types on the report, need to capture that as well"* — *"tender
types is in columns"* — and later the same day: *"nothing on cash collected either"*.

**THE FILE, measured** (kept in the private bucket `onboarding-intake`, path `f4f1c16e-…/pos_rq_pos_product_sales/sales-by-invoice-report.xlsx`,
1.7 MB, 10,823 rows, 41 columns): ONE ROW PER INVOICE — Created On, Invoiced By, Invoiced At, Tendered By (+ username), Sold By (+
username), Invoice #, Customer, Application, Invoice Comments, Invoice Subtotal, Adjustments, Net Sales, Sales, Total Cost, Gross
Profit, Extra Charges, Total Donations, Invoice Total, Total Coupons, Gift Card Sales, Non-Revenue Sales, Channel, Region, District,
Emailed — then TWELVE AMOUNT COLUMNS, ONE PER TENDER TYPE (the card brands, their non-integrated twins, Cash, Debit PIN, a vendor
rebate applied as payment) — then TotalTaxPaidAmount and one named jurisdiction tax column. `Invoice #` is the `trans_id` the
line-level export carries (raw_sales, 48,875 rows for this org).

**THE MIS-CARDING, stated plainly.** On 2026-09-20 this file was committed as the BY-PRODUCT aggregate (`pos_product_sales`); it is
not a per-product report, it is a per-invoice report. Its instance `pos:rq:pos_product_sales` still reads "verified"; the rows it
wrote into `raw_sales` were removed on 2026-09-21; at 01:58Z the owner re-committed it and **`raw_sales_product` now holds its
10,823 invoice-level rows** (source `pos_product_sales`). Nothing of its tenders is captured anywhere. The recovery is the intake's
own clicks (the PR body): retire the stale line (with the COUNTED removal of the 10,823 rows, or the owner-approved SQL surfaced
there), add the invoice card, re-read the kept file under it — never a DB edit.

**THE DECISION ON MIG 1011's TABLE — option (b), new tables; `raw_sales_product` KEPT for a genuine per-product export.** The
by-product card describes a real report at the line grain; an invoice header plus a VARIABLE set of tender columns is two grains
and needs a child table; repurposing would re-point the product card at nothing. Nothing is dropped (the emptiness precondition a
repurposing migration would need is written into 1012's header so it is on the file, not in a head).

**THE CLASSES (CLAUDE.md "A fix is a DESIGN fix"), each fixed for every caller, one home each:**

| # | The class | The mechanism (ONE home) |
|---|-----------|--------------------------|
| 1 | **A report kind that is not a card is filed under the nearest one and its own facts are lost.** | **`sales_by_invoice`** in the report-kind registry (mig `1012` house row; `report_kinds.HOUSE_KINDS` mirror; `SEED_MIGRATIONS` = every seed file, parsed back by `harness_report_kinds.py` §A): landing `invoice`, layout `sales_by_invoice` (`column_mapping.TARGET_FIELDS`), `requires_columns` Invoice Total, `excludes_columns` SKU / Tracking # / Product Name / IMEI / Serial. Detection: the kind's own signals rank it FIRST but within the ask margin of the by-product card (both are the same POS's sales exports and share headers) → ASK naming both, never a silent assignment; the migration seeds the house `report_signature` of the measured 41 headers (header names only) → CONFIRM at 1.0; `looks_like_sentence` names the card and the page |
| 2 | **The tender columns are not fixed** (a POS names them per tenant and register set-up), so no layout can map them. | **Step 2.5b — "Which columns are tender types, and what kind of payment is each?"** (`onboarding_intake.STEP_TENDER_COLUMNS`, `frontend …/tender-columns-step.tsx`): `invoice_tenders.suggest_columns` (PURE — classifiers INJECTED) proposes a role for every money column that is not a header field — a tender with its class from the words in the header, a tax component, or unplaced (listed, never guessed) — with Σ / cells / non-zero counts and the keyed-by-hand flag (`closing.router.keyed_manually`); `apply_decisions` lays the person's decisions over (an unknown class is a refusal); the decisions ride the auto-saved draft (`tender_columns`) and the commit REMEMBERS them as the org's `closing_tender_map` rows with report `invoice` (`tender_map_rows` → `_intake_write_tender_map`, read back through the resolver) — "your earlier choice" next month |
| 3 | **A third tender vocabulary** (the X-report list, `_canon_tender`, mig-944's bill-pay tokens — and now the invoice columns). | **`closing.router.TENDER_VOCAB`** — THE one list: the closing axis (seven, `closing_axis`) + the finer invoice classes `debit` / `coupon` / `vendor_rebate`, each with its `recon_class` (cash / card / other gate) and `folds_to` on the axis; `CANON_TENDERS` / `CANON_TENDER_LABEL` are DERIVED; `tender_class(raw)` is the one header / label ladder; **`_canon_tender` = `fold_to_axis(tender_class(raw))`** — byte-identical on a spelling battery (`harness_sales_by_invoice.py` A7) with ONE stated delta: the spelled-out brand *American Express* answered None (unmapped) and now answers credit like its abbreviation; `_xr_canon_known` still rejects coupon / vendor-rebate labels (they fold to None). The pre-existing siblings are NAMED in the lock's allow set with the reason each is a different question (`_XR_TENDERS` / `_XR_CARD_WORDS` — the X-report's cash/card/other stamp; `_CARD_HINTS` — the B2B money gate; `metric_recon.DEFAULT_CARD_TENDERS`; `merchant_portals._BRAND_SYNONYMS`; `tender_config.STANDARD_DEFS` / `tender_axis`'s hardcoded rclass — seams) |
| 4 | **The landing.** | Parent + child through **`_ingest_mapped_df` twice** (`_intake_land`, `invoice`): the invoice header via `TABLE_MAP['sales_by_invoice']`, the tender rows (`invoice_tenders.tender_rows`: one per (invoice, column) with amount ≠ 0; a tax component rides the same grain, role `tax`) via **`column_mapping.CHILD_TABLE_MAP`** (the one home of "which child table"; `landing_identity.KIND_STAMP` + `ingest_slice.INGEST_PARTITION` name both tables, the lock pins they agree); both stamped `sales_by_invoice`, both slice-replaced store × dates × kind, both re-read (`_intake_reread_invoice` / `_intake_reread_invoice_tenders`); `ok` only when parent rows = built = inserted AND child rows = built = inserted AND Σ tie AND Σ tenders re-read = shown. Pre-1012 a landing is REFUSED naming the file. A blank class at the frame boundary is `''`, never pandas' `NaN` → `'nan'` (caught by the proof, D10) |
| 5 | **The tie-out.** | 2.5: Σ of the tie field (`KIND_FIELDS['invoice'].amount` = `net_sales`; `tie_field` ∈ `INVOICE_TIE_FIELDS`) beside the file's total; 2.5b: **Σ of the declared tender columns vs Σ invoice total** per invoice, in words (`invoice_verify` → `tender_tie` → `stage2_refusals(tender_tie=)`: refused unless attested); the Σ tax + the labelled components |
| 6 | **Reconciliation, not a sibling.** | Stage 4 shows the tender split per store-day BESIDE the X-report: **`_intake_invoice_tender_recon`** reads the invoice leg through `closing.router._invoice_tenders_by_store` and the X-report leg through `_tender_split_by_store(basis='x_report')` (the closing module's own readers + `_addr_resolver`), `invoice_tenders.recon_vs_xreport` states matches / differences per gate / "no X-report for this day"; `verify_table.tender_note`; the report links pair the export by invoice number (`trans_id` ↔ `raw_sales.trans_id`), store, rep and date (`report_links.columns_for('invoice', …)`) and with the X-report by store + date |
| 7 | **"Nothing on cash collected"** — the closing recons read ONE basis (the X-report) and a tenant without one read nothing. | **THE TENDER BASIS IS CONFIG**: `storeops.tenants.closing_tender_basis` (mig 1012; NULL = house `x_report`, byte-identical), `invoice`, `x_report_else_invoice`. ONE resolver **`closing.router._tender_split_by_store`** (`_xreport_tenders_by_store` keeps its name and shape = its totals; `basis_by_store` says the leg per store) dereferences `tender_basis`; the invoice leg is `_invoice_tenders_by_store`; the X-report leg `_xreport_rows_by_store`. The three consumers (`_closing_summary_for_date` — `money_recon.tender_source` = the leg read, `deposit_recon_report`, `_pos_tenders_for_days`) call it; every payload carries `tender_basis_info` and the pages state it (`DailyClosingVerify` labels the leg; cash-recon-management and the pickups carry the line). The intake's 2.5b asks *"Do you also upload a daily cash register / X-report? If not, the invoice tenders will be your cash-collected basis"* and the commit writes the answer through **`put_tender_basis`** (the one writer; read back; refused if it did not stick) |
| 8 | **A mis-filed line could only be fixed by editing the DB.** | **`POST /onboarding/intake/retire`**: status `retired` (a new `STATUSES` member) with the reason and the name; out of the verify table / runbook / sign-off, listed under `rail.retired` with its kept file; `remove_landed` = a DRY RUN counting the rows the line landed (its recorded slice, parent + child) → repeated with `confirm_rows` = that count removes exactly them (409 if the count moved), recorded on the row and in `upload_trace`. The kept file is re-read under the right kind with `use_stored=1` + the retired line's key and the reference is CARRIED to the new instance (`_intake_carry_kept_file`) |

**DUPLICATE CHECK (build gate) — searched** §2 (`_ingest_mapped_df`, the mig-1004 shape rules, `_intake_land`), §12 / §23h (`pos_tender_summary`,
`_canon_tender`, `CANON_TENDERS`, `tender_config.make_resolver` / `tender_axis`, `closing_tender_map` mig 111, `closing_recon_mode` on
`storeops.tenants`), §16 (`raw_sales_product` mig 1011, `report_kind` / `report_signature` mig 1010, `accessory_config.billpay_card_tenders`
mig 944), §23b (`_addr_resolver`, `_xreport_tenders_by_store`), §23f (`tax_collected.aggregate`, mig 991), §25.11–25.12, §30.6–30.12, §32.
**REUSED:** `_ingest_mapped_df` (twice — no new insert), `landing_identity` / `ingest_slice` / `column_mapping.TABLE_MAP`, the report-kind
registry + `detect_report_kind` + `learn_signature`, `closing_tender_map` + `make_resolver` (one more leg), `_addr_resolver`,
`_xreport_tenders_by_store` (now the resolver's totals), `report_links`, the intake spine (auto-save, the shared resolver panel, Stage 4 / 5),
`upload_trace`. **NEW:** `invoice_tenders.py` (pure), the two tables + one column (mig 1012), `sales_by_invoice` (registry row + layout +
`CHILD_TABLE_MAP`), `TENDER_VOCAB` / `tender_class` / `tender_basis` / `put_tender_basis` / `_tender_split_by_store` / `_invoice_tenders_by_store`
(closing), `/onboarding/intake/retire`, step 2.5b, the lock. **Not a sibling:** no second tender list, no second store-day derivation
(the intake's tie-out reads the closing module's reader), no second basis reader, no second slice rule.

**PROPOSED, NOT BUILT (finance-agent territory).** `tax_collected.aggregate()` (§23f, mig 991 — the `sales_tax_payable` liability) reads
line-level `raw_sales.tax`, which this org's line-level export does not carry. Proposal: `SALES_COLUMNS` gains nothing; the endpoint and
`statement_engine.build_inputs_full` pass a second row source — `raw_sales_invoice` rows as `{trans_id, trans_date, store, ext_price:
net_sales, tax, voided: '', trans_type: ''}` — ONLY for (org, period) slices whose line-level rows carry NO tax (`blank_fields_over(rows,
['tax'])` — the §32 signal), keyed so an invoice number present on a line-level row with tax is never counted twice (the double-count
guard: a set of `trans_id` with line-level tax > 0 excludes the invoice row). The config key it needs: `metric_source_of_truth` metric
`sales_tax` source `line_level` \| `invoice_level` \| `line_else_invoice` (house default `line_level`, byte-identical). Until then the
Stage-4 note says *"the Tax Collected report does not read it yet"* and `CONSUMERS` does not name it.

**Proof.** `backend/harness_sales_by_invoice.py` (95, DB-free, the intake fake client driving the REAL router + closing): §A the vocabulary
(derived axis, the ladder on the measured headers, the byte-identity battery + the one delta, the mig-111 resolver per leg); §B the suggestion
engine over the 41 measured headers (twelve tenders with classes, the keyed twins, the tax component, header fields never proposed, unplaced
listed, decisions laid over, unknown class refused, the map round-trip); §C detection (ask before the seed, confirm 1.0 with the 1012 signature
parsed out of the SQL, the wrong-file sentence, the by-product card not chosen); §D end to end (analyze → 2.5b block → commit → re-read = built =
inserted, the class per row, the tax rows, the map remembered and read back as "your earlier choice", a re-commit replaces the slice in BOTH
tables, raw_sales / raw_sales_product untouched); §E Stage 4 beside a fake X-report (matches / differences / a day without), the note, the
links; the BASIS: default = the X-report byte-identical, `invoice` = the derived split, `x_report_else_invoice` per store-day, the one writer,
the intake sets it; §F the mis-carding reproduced (the line-level card refuses the file before a row is written), the retire path (dry run →
409 on a moved count → confirmed removal of exactly the slice; the kept file re-read under the new kind and carried; the commit from it);
§G negative controls, registration, RULE TWO, the migration, CI. Lock `backend/harness_tender_vocab_lock.py` (14: one home, no second
ladder, the named siblings, the resolver, nine negative controls). **Pins CHANGED (stated):** `harness_report_kinds.py` §A parses every
`SEED_MIGRATIONS` file; `harness_onboarding_intake.py` counts 7 Save rows on stage 2; `harness_onboarding_intake_d.py` A5 — the spine's
`next_step('2.5a')` is `2.5b` (the page walks a sales export straight to 2.6 by kind); `harness_landing_identity_lock` (b) pins the child-table
map; `harness_report_links_lock` pins the invoice branch. Unchanged and green: landing_identity 54, report_kinds 121, intake 164 / B 87 / C 92 /
D 34, line_class 35 + lock 15, tender_recon_3way 54 (the byte-identity pin of the default basis), xreport_tender_vocab 63, tax_collected 82,
closing_submissions 34, report_links 77 + lock 15, report-kind lock 20, landing-identity lock 32, mapping-key 17, org-scope 25, lineage 63,
the carrier-vocab guard; `prove_report_kinds.mjs`, `prove_pos_term_copy.mjs`, `tsc --noEmit`.

**Files.** `commcalc/invoice_tenders.py` (NEW, pure), `closing/router.py` (`TENDER_VOCAB` …, `tender_basis` / `put_tender_basis` /
`tender_basis_info`, `_tender_split_by_store`, `_invoice_tenders_by_store`, `_xreport_rows_by_store`, the three consumers), `commcalc/router.py`
(the invoice branches of prepare / land / commit / links, `_intake_tender_columns_block`, `_intake_tender_resolver`, `_intake_write_tender_map`,
`_intake_invoice_tender_recon`, `_intake_reread_invoice(_tenders)`, `onboarding_intake_retire`, `_intake_landed_slice_of`, `_intake_remove_landed`,
`_intake_carry_kept_file`), `onboarding_intake.py` (the `invoice` kind, step 2.5b, `STATUS_RETIRED`, `rail.retired`, `tender_note`),
`column_mapping.py` (`sales_by_invoice`, `CHILD_TABLE_MAP`), `landing_identity.py`, `ingest_slice.py`, `report_kinds.py` (`SEED_MIGRATIONS`, the
row, landing `invoice`), `data_lineage_registry.py` + `925` (edges 139 / 140), mig `1012_sales_by_invoice.sql` (NOT applied), frontend
`onboarding/intake/tender-columns-step.tsx` (NEW), `stage2.tsx`, `page.tsx`, `intake-shared.tsx`, `lib/report-kinds.ts`,
`components/DailyClosingVerify.tsx`; `.github/workflows/carrier-vocab-guard.yml`; `docs/ONBOARDING_FLOW_DESIGN.md` §11.

**Seams left (reported, not hidden).** (1) `tender_config.tender_axis` hardcodes the recon class of the built-in seven (a copy of
`TENDER_RECON_CLASS` restricted to the axis) and `STANDARD_DEFS` seeds the axis — both named in the lock; dereferencing them is a
closing-agent change. (2) The 3-way tender recon's X-report leg reads `pos_tender_summary` per tender KEY by design (a per-tender leg beside
the closing and sales legs) — the basis switch is the closing recons', not the 3-way's; under `invoice` the 3-way still shows the X-report leg
as absent (honest). (3) `atu_opportunity.CARD_TOKENS` is a pre-existing card-tender gate for one report, named in the lock. (4) The
invoice-level tax dereference in `tax_collected.aggregate` is proposed above, not built. (5) Detection without the seeded signature is an ASK
between the invoice card and the by-product card — adding `excludes_columns` (Invoice Total) to the by-product card is a 1010 seed change the
mirror pin forbids without a migration of its own. (6) The DM-verify page labels the leg; the cash-recon-management and pickups pages carry
`tender_basis` in their payload but render no line yet — the closing agent's PR.
