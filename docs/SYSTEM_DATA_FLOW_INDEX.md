...
| `GET /commcalc/onboarding/intake/line-class?instance_key=` (2.5a, read-only: the instance's LANDED slice re-read as the commit re-read it → `line_class` block: current rules + what they classify, `gate_open`, the proposal per class with counts, the metric buckets' match / proposal) · `PUT /commcalc/onboarding/intake/line-class` (`fields`, `tokens`, `metric_rules`, `no_activations`, `by` → `put_accessory_config(activation_details_rules)` + `put_exec_metric_config` per LINE bucket, then the slice RE-COUNTED through the predicate onto `verified_numbers.activation_classes` / `activation_gate`; status verified when the landing was ok and the gate is closed; gated like every Classification edit) | `router.onboarding_intake_line_class` / `onboarding_intake_put_line_class` → `_intake_landed_slice` → `_intake_reread_sales` (now also `contract_type`) → `_intake_line_class_block` (`line_class.suggest_rules` / `suggest_metric_rules` / `count_classes`) | §30.12; proof `harness_onboarding_intake_d.py` (34) |
