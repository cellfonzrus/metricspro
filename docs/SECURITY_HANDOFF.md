...
**Last updated:** 2026-08-17 (item 15 pt53 — core/router roles + settings) · **Branch:** `claude/employee-commission-structure-3g5hva` · **PR:** #30 (draft, CI green)
...
| **Item 15 — Pydantic rollout** | 🟡 in progress, incremental (parts 1–53 = 341 endpoints; ~92 remain). pt53: core create_role/update_role/put_security_settings/put_tenant_settings. Remaining core/router = ~20 auth-credential handlers; `create_login`/`assign_role` are called IN-PROCESS by hr onboarding_provision (+ thread into helpers) → convert callee+caller together or defer; the rest are pure named-readers (connect_tenant, disable_and_switch, reinstate_login, resend_invite, reveal_code, admin_set_password, set/verify_phone, reset_password, reset_tenant_admin_password, bulk_provision, delete/deactivate/purge_user, signup, bulk_assign) — verify no in-process callers, then convert. Fully swept: commcalc, hr(+letters), crm-leads, referral, notify, storevisit, billing, recovery, remediation, asset(PO+router), closing(27/28), account, storeops/payroll_approval, core small files, core/fix_pipeline. core/router: failures cluster + portal-reports + widget-overrides done; **~24 raw remain = auth/tenant-credential handlers** (signup, create_login, assign_role, bulk_assign, connect_tenant, admin_set_password, reset_password, set/verify_phone, put_tenant_settings, put_security_settings, create/update_role, resend_invite, reveal_code, bulk_provision, delete/deactivate/purge_user, disable_and_switch, reinstate_login, reset_tenant_admin_password) — security-critical, MANY thread body into auth helpers → read each carefully, expect a mix of convert + defer. Deferred: whats_new+training (clean_* threaders), impersonation put_policy + core support_docs + create_fix_request (else body / helper-threaders), commcalc-20/helpdesk/hr-3/crm-3 (classified earlier), pos (40, skipped). |
