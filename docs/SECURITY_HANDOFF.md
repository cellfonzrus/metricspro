...
**Last updated:** 2026-08-17 (item 15 pt54 — core/router credential handlers) · **Branch:** `claude/employee-commission-structure-3g5hva` · **PR:** #30 (draft, CI green)
...
| **Item 15 — Pydantic rollout** | 🟡 in progress, incremental (parts 1–54 = 354 endpoints; ~79 remain). pt54 (core/router credential handlers): set_phone, verify_phone, reveal_code, reset_password, reset_tenant_admin_password, bulk_provision, delete_user, deactivate_user, admin_set_password (write-only `password` stays Any), connect_tenant, disable_and_switch (reuses ConnectTenantIn), reinstate_login, resend_invite. Remaining core/router raw: **signup, purge_employee, bulk_assign** (verify + convert), and **create_login / assign_role** (called IN-PROCESS by hr onboarding_provision — `core_create_login({...})` / `assign_role({...})` — AND assign_role threads into `_normalize_grant_write`; convert callee+caller together or defer). Deferred: create_fix_request + support_docs_upsert/import (helper-threaders); whats_new+training; impersonation put_policy; commcalc-20/helpdesk/hr-3/crm-3; pos-40 (skipped). Fully swept: commcalc, hr(+letters), crm-leads, referral, notify, storevisit, billing, recovery, remediation, asset(PO+router), closing(27/28), account, storeops/payroll_approval, core small files, core/fix_pipeline. core/router: failures cluster + portal-reports + widget-overrides done; **~24 raw remain = auth/tenant-credential handlers** (signup, create_login, assign_role, bulk_assign, connect_tenant, admin_set_password, reset_password, set/verify_phone, put_tenant_settings, put_security_settings, create/update_role, resend_invite, reveal_code, bulk_provision, delete/deactivate/purge_user, disable_and_switch, reinstate_login, reset_tenant_admin_password) — security-critical, MANY thread body into auth helpers → read each carefully, expect a mix of convert + defer. Deferred: whats_new+training (clean_* threaders), impersonation put_policy + core support_docs + create_fix_request (else body / helper-threaders), commcalc-20/helpdesk/hr-3/crm-3 (classified earlier), pos (40, skipped). |
