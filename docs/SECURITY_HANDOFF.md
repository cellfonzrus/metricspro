...
**Last updated:** 2026-08-17 (item 15 pt15) · **Branch:** `claude/employee-commission-structure-3g5hva` · **PR:** #30 (draft, CI green)
...
| **Item 15 — Pydantic rollout** | 🟡 in progress, incremental (parts 1–15 = 110 endpoints; ~323 remain) |
...
- **Item 15 (Pydantic), continue incrementally.** Done: parts 1–7c (containment/export/auth-ledger
  ...
  **storeops done** except 7 body-threading config helpers (deferred by rule). **Part 8:** helpdesk
  ...
  connector-schedule, accessory-flags-push. Next commcalc candidates: the money-sensitive plan/category
  settings group (put_commission_settings, save_plan_installment, put_activation_matcher/plan_line_matcher,
  put_category_qualification, save_category_rule, put_category_payout, expected-commission promote/revoke,
  update_plan_installment, mrc_mapping_confirm/bulk_classify, save_payout_schedule, save_product_mrc,
  carrier_template_clone, save_distributor — read each first, watch for body-threading). hr (~32,
