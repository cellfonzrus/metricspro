...
**Last updated:** 2026-08-17 (item 15 pt14) · **Branch:** `claude/employee-commission-structure-3g5hva` · **PR:** #30 (draft, CI green)
...
| **Item 15 — Pydantic rollout** | 🟡 in progress, incremental (parts 1–14 = 107 endpoints; ~326 remain) |
...
- **Item 15 (Pydantic), continue incrementally.** Done: parts 1–7c (containment/export/auth-ledger
  ...
  **storeops done** except 7 body-threading config helpers (deferred by rule). **Part 8:** helpdesk
  ...
  item-category, item-mapping upsert+bulk, device-model add). Next commcalc candidates: connector-schedule,
  create-custom-import-type, accessory-flags-push, then the money-sensitive plan/category settings group
  (put_commission_settings, save_plan_installment, put_activation_matcher, put_category_payout,
  expected-commission — read each first). hr (~32,
