...
**Last updated:** 2026-08-17 (item 15 pt7h) · **Branch:** `claude/employee-commission-structure-3g5hva` · **PR:** #30 (draft, CI green)
...
| **Item 15 — Pydantic rollout** | 🟡 in progress, incremental (parts 1–7h = 59 endpoints; ~374 remain) |
...
- **Item 15 (Pydantic), continue incrementally.** Done: parts 1–7c (containment/export/auth-ledger
  ...
  org-structure + approvals + config + payroll + payroll-config + action-plans + google-reviews +
  schedule-templates + shift-extension + face-deletion + store-config lax). Next candidates by size:
  commcalc (125), storeops (~24 left), hr (32), helpdesk (29). **POS skipped** — module incomplete /
  no data. **Rules:** skip
  endpoints that
