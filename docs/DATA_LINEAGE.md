...
- **VidaPay MA feeds are also the PAID side of the activation discrepancy report** (mig 312, owner spec 2026-09-01 Phase C — "MA TX can also be used to verify the sales ingested via the B2B"): `ma_recon.run_ma_discrepancy` (triggered by `POST /discrepancy/run` for plan-mode orgs) verifies every B2B activation (the `SALES_DISPLAY_SOURCES` union, non-blank `contract_type`, digit-normalized `serial_1`) against `raw_ma_commission` spiff evidence **∪** `raw_ma_daily_tx` month-1/activation-order evidence via the same two-hop serial → `activation_order` → `order_number` join. Sold-but-unpaid activations land in `commcalc.discrepancy_results` (`source='ma'`, `comp_type='MA_ACTIVATION'`) attributed through the uploadable business rules in `commcalc.ma_payment_rule` (first match by priority; `expected_outcome` steers the report tab), or with the literal reason **'no business rule configured'** when no rule matches — absence of a rule is itself reported, never papered over. Each row carries an `evidence` JSONB naming exactly which source had/lacked the activation (b2b `trans_id`/date, ma_commission `activation_order`/imei, ma_tx `order_number`). The MA writer delete-then-inserts only its own `(org, period, source='ma')` slice — the Boost engine's rows are never touched.
