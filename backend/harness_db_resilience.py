...
# Re-pinned 1580 -> 1581 on 2026-09-11. The delta is exactly ONE endpoint and nothing was removed:
#     + /account/device-payable                       GET
# Device Payable as at a date (owner directive; index §23z) — a new READ-ONLY report mount over the
...
# Re-pinned to 1590 on 2026-09-20, after merging origin/main. The arithmetic, all measured (not
# assumed) with `git stash` on this tree:
#     1584  this branch before this package        (the pin still read 1583 — an EARLIER package on
#                                                   the branch added a route without re-pinning, so
...
# Re-pinned to 1594 on 2026-09-20 after merging origin/main again. The alert/autofix package on this
# branch adds NO route either (measured: `git diff 3281954 bd058898 -- '*.py'` contains not one added or
# removed @router/@app method decorator). The +1 is origin/main's #254, the onboarding-intake Stage C
