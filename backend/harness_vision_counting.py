...
# ══════════════════════════════════════════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════════════════════════════════════════
# WIRING — the ways a correct GateCounter is still defeated by the code around it
# ══════════════════════════════════════════════════════════════════════════════════════════════
def wiring():
    """counting.py being right is not enough.

    A GateCounter holds per-track state — which side of the corridor each track is committed to —
    so it has to OUTLIVE a frame. Build it inside the detect loop and the hysteresis resets every
    tick: the counter is then perfectly correct and permanently reads zero. The analyzer caches
    gates and rebuilds only when the zone set changes; this is why."""
    print()
    print("(11) WIRING — a correct gate, defeated by the code around it")

    LINE = {"kind": "line", "is_active": True, "id": "z1", "inward": "left",
            "geometry": {"x1": 0.5, "y1": 0.0, "x2": 0.5, "y2": 1.0}}
    # One track walking left to right, far enough past BOTH test lines to commit on the far side.
    # An earlier version of this test stopped at 0.80, which is one observation past a line at 0.7 —
    # and the gate correctly refuses to fire on one, since `confirm_frames` is 2. The test was
    # wrong, not the gate.
    STEPS = (0.20, 0.32, 0.44, 0.56, 0.68, 0.80, 0.92)

    def box(x):
        return {"x": x - 0.04, "y": 0.5, "w": 0.08, "h": 0.30}

    kept = CNT.gates_for([LINE])[0]
    got = [d for i, x in enumerate(STEPS)
           if (d := kept.update("t1", (x, 0.80), box(x), now=i * 0.17))]
    check("a gate that persists across frames counts the crossing exactly once",
          got == ["out"], f"got {got}")

    # THE BUG THE CACHE PREVENTS. A fresh gate each frame has never seen a previous side, so it can
    # never commit one, and nothing is counted — silently, for ever.
    per_frame = [d for i, x in enumerate(STEPS)
                 if (d := CNT.gates_for([LINE])[0].update("t1", (x, 0.80), box(x), now=i * 0.17))]
    check("rebuilding the gate every frame counts NOTHING, and says nothing",
          per_frame == [], f"got {per_frame}")

    check("an inactive line makes no gate",
          CNT.gates_for([{**LINE, "is_active": False}]) == [])
    check("a polygon zone makes no gate", CNT.gates_for([{**LINE, "kind": "polygon"}]) == [])
    check("no zones makes no gates", CNT.gates_for([]) == [])
    check("None makes no gates", CNT.gates_for(None) == [])
    # Every camera in the live estate is flagged as an entrance and none has a line drawn. That
    # combination counts nothing for ever and warns nobody, so it is asserted rather than folklore.
    check("an entrance camera with no line zone yields no gates — which is why it reports zero",
          CNT.gates_for([{"kind": "polygon", "is_active": True}]) == [])

    two = CNT.gates_for([LINE, {**LINE, "id": "z2",
                                "geometry": {"x1": 0.7, "y1": 0.0, "x2": 0.7, "y2": 1.0}}])
    check("two active lines make two gates", len(two) == 2)
    both = [[d for i, x in enumerate(STEPS)
             if (d := g.update("t1", (x, 0.80), box(x), now=i * 0.17))] for g in two]
    check("...each with independent state, each counting its own crossing",
          both == [["out"], ["out"]], f"got {both}")
