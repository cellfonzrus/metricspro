"""THE pure pricing logic for supply ordering — ONE copy, used by the backend AND the standalone kit.

Home: backend/app/modules/supply/pricing_core.py (index §34 / §36). It was tools/vendor_price_compare/
pricecompare/core.py in phase 1; phase 2 moved it here so the platform and the kit the owner runs on their
own machine can never drift. The kit loads THIS file by relative path when run from the repo
(tools/vendor_price_compare/pricecompare/_shared.py) and tools/vendor_price_compare/build_zip.py bundles it
into the downloadable zip. `backend/harness_vendor_price_compare.py` §I fails the build if a second copy of
these functions appears anywhere in the repo.

No browser, no network, no files, no app imports (stdlib only) — so it loads on the owner's laptop too.
...
No vendor name, URL or selector appears here: vendors are rows (commcalc.po_vendor / vendors.json).
"""
