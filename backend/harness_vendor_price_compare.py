"""PROOF — the vendor price comparison kit's pure logic (tools/vendor_price_compare/pricecompare/core.py).
...
KIT = os.path.join(HERE, "..", "tools", "vendor_price_compare")
sys.path.insert(0, KIT)

from pricecompare import core  # noqa: E402
...
for fn in ("pricecompare/core.py", "pricecompare/scrape.py", "pricecompare/report.py", "compare_prices.py"):
    src = open(os.path.join(KIT, fn)).read().lower()
    bad = [h for h in hosts if h.split(".")[-2] in src] + [v["key"] for v in cfg["vendors"] if v["key"] in src]
    check(f"G1 no vendor in {fn}", not bad, bad)
...
from pricecompare import scrape as _s  # noqa: E402  (imports no browser at module load)
