...
def house_tables():
    ...
    return {
        ...
        "accessory_config": [],                               # no row → defaults, apply_to_gp False
...
check("1b. legacy buckets: Ondigo → Acc GP", close(row_h.get("acc_gp"), 12.0))
...
check("4a. accessory BY CATEGORY: HandsetBranded + Accessories → Acc GP 17", close(row_l.get("acc_gp"), 17.0))
...
check("9d. totals include the MA row", close(res_l["totals"]["comm"], 356.14)
      and close(res_l["totals"]["atu"], 120.25) and close(res_l["totals"]["acc_gp"], 17.0))
...
check("7. rep rows use the same per-line rule (Amy acc 17 / phones 100 / plan 7)",
      close(rep_amy.get("acc_gp"), 17.0) and close(rep_amy.get("phone_sales"), 100.0)
      and close(rep_amy.get("plan_gp"), 7.0))
