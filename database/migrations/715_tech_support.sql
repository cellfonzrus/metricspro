...
VALUES
  ...
  ('00000000-0000-0000-0000-000000000001', '/commcalc', 'Commissions Dashboard', 'commissions',
   ...
   '[{"symptom":"Commissions show $0 or empty for a period","diagnosis":"Wrong sales file (the 25-col legacy export has no Contract Type) or a period with no upload.","fix":"Re-upload the full 78-col Sales Transaction Details under Data Imports and re-run the calculation.","escalate_when":"The correct 78-col file is uploaded for a period that has data and reps still show $0."},
     {"symptom":"A non-Boost tenant shows $0 commissions","diagnosis":"carrier_mode gate: non-Boost tenants pay only from Commission Plans, so a missing plan assignment yields $0.","fix":"Assign the rep or store a Commission Plan. $0 with no plan is expected, not a bug.","escalate_when":"A plan is assigned and active but pay is still $0."},
     {"symptom":"Recompute returns a 502 gateway timeout","diagnosis":"calculate/{period} exceeds about 300s and Railway 502s, but the job completes anyway.","fix":"Do not re-fire; poll the period commissions after a few minutes.","escalate_when":"The period is still empty well after the run should have finished."}]',
  ...
  ('00000000-0000-0000-0000-000000000001', '/closing', 'Daily Closing', 'closing',
   ...
   '[{"symptom":"Reconciliation flags every tender line","diagnosis":"The POS X-Report (the tender source) was not ingested for that store and day, so nothing matches.","fix":"Confirm the X-Report import for the store and business day, then re-run recon.","escalate_when":"The X-Report is confirmed ingested and lines still do not match."},
     {"symptom":"A closing is blocked on submit","diagnosis":"Cash short or credit over is a hard block; cash over and credit under only flag.","fix":"Recount and correct the tender, or a manager reviews it on Management Review.","escalate_when":"Amounts are confirmed correct but the block persists."},
     {"symptom":"An unexpected ops chargeback appeared for a closer","diagnosis":"A missed daily closing flags the effective closer (retail-ops); this is by design.","fix":"Review the chargeback at payroll and post or waive it per policy.","escalate_when":"The closer completed the closing but was still charged."}]',
