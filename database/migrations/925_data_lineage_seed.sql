...
('store_performance_report','b2b Store Performance scorecard','Custom import → raw_custom_import','raw_custom_import','commcalc.raw_custom_import','Data Imports','ingest',true,'router.py:_ingest_custom_report','Per-store scorecard captured as JSONB; detected by signature (Acc Ext Price + Bill Payment Qty).',9),
('store_performance_report','Store Performance (accessory store breakup)','—','accessory_store_breakup','Per-store Acc Ext Price / Acc GP','Custom Report','display',true,'router.py:_cr_resolve_store_performance','The per-store accessory breakup Sales-by-Product lacks — Acc Ext Price / Acc GP per store, plus per-store Activations + Bill Payments; group by Division/Region to split brands.',46),
