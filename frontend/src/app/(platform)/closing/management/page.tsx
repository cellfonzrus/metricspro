                ...
                {isOpen && (
                  <div style={{ borderTop: '1px solid var(--border)', padding: '10px 14px' }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 8 }}>
                      Tender split (final try): cash {fmt(lastTry(g).t_cash)} · credit {fmt(lastTry(g).t_credit)} · ext CC {fmt(lastTry(g).t_ext_cc)} · gift {fmt(lastTry(g).t_gift)} · store acct {fmt(lastTry(g).t_store_acct)} · zelle {fmt(lastTry(g).t_zelle)}
                    </div>
                  </div>
                )}
              </div>
