            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <tbody>
                  ...
                  <tr><td style={td}>Units sitting on an invoice whose header says <strong>VOIDED</strong> — counted in the figure above, because the Device Purchases report counts them too and two finance reports disagreeing about voided invoices is worse than either rule. The amount is published here so the rule can be decided on a number</td><td style={tdr}>{num(T.voided_invoice_payable_devices)} · {money(T.voided_invoice_payable_amount)}</td></tr>
                  <tr><td style={td}>Payable as it would read if voided invoices were excluded</td><td style={tdr}><strong>{money(T.payable_excluding_voided)}</strong></td></tr>
