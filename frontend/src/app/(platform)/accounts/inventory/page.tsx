...
export default function InventoryValuesPage() {
  ...
  async function saveCfg() {
    ...
    try {
      ...
      setCfg(c); setPass(''); setMsg(`${pos} connection saved.`)
    } catch (e: any) { setMsg('Save failed: ' + (e?.message || e)) }
    setSavingCfg(false)
  }
  ...
  function buildPayload(): ExportPayload {
    return {
      title: 'Inventory Values — Balance Sheet', subtitle: `${pos} Inventory Aging · editable`,
      filename: 'inventory-values',
      sheets: [{ name: 'Inventory', rows, columns: [
        { header: 'Store', get: (r: Row) => r.store },
        { header: `Swept (${pos})`, get: (r: Row) => r.swept_value, money: true },
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, gap: 12, flexWrap: 'wrap' }}>
        <div>
          ...
          <p className="pg-note" style={{ color: 'var(--text2)', fontSize: 14, margin: '4px 0 0' }}>Real-time on-hand value per store ({pos} Inventory Aging) → Balance Sheet · a manual override always wins.</p>
          ...
          <p style={{ color: 'var(--text3)', fontSize: 12.5, margin: '6px 0 0', maxWidth: 780 }}>
            ...
            counts. For “do our device COUNTS match {pos},” use{' '}
            <Link href="/commcalc/asset/inventory-recon">On-Inventory ↔ {pos} Recon</Link>.
          </p>
        </div>
      ...
      <div className="card" style={{ padding: 14, marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginTop: 10 }}>
          <input style={{ ...inp, width: 180 }} placeholder={`${pos} username`} value={cfg.portal_user || ''} onChange={e => setCfg({ ...cfg, portal_user: e.target.value })} />
          <input style={{ ...inp, width: 180 }} type="password" placeholder={cfg.has_credentials ? '•••••• (unchanged)' : `${pos} password`} value={pass} onChange={e => setPass(e.target.value)} />
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 760 }}>
            <thead>
              <tr style={{ fontSize: 11, color: 'var(--text2)', textTransform: 'uppercase' }}>
                ...
                <th style={{ textAlign: 'right', padding: '8px 12px' }}>Swept ({pos})</th>
      ...
      <p style={{ fontSize: 12, color: 'var(--text3)', marginTop: 10 }}>
        The Balance Sheet inventory line uses the <strong>effective</strong> value per store (manual override if set, else the swept {pos} value; stores with neither fall back to the asset-ledger on-hand value). After editing, re-run <Link href="/accounts">Compute statements</Link>.
      </p>
