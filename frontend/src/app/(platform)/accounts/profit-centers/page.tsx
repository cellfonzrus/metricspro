...
export default function ProfitCentersPage() {
  ...
  useEffect(() => { load() }, [load])
  const centers = (data?.centers || []).filter((c: any) => c.center_type === 'profit')
  const mapOf: Record<string, string> = Object.fromEntries((data?.store_map || []).map((m: any) => [m.store, m.profit_center_code]))

  useEffect(() => {
    if (!pc) { setStmt(null); return }
    let alive = true
  ...
  const save = (row: any) => api('/api/v1/account/centers', { method: 'PUT', body: JSON.stringify({ ...row, center_type: 'profit' }) })
  ...
  return (
    <div style={{ padding: 16, maxWidth: 1200 }}>
      ...
      {data?.problems?.profit?.length > 0 && <div style={{ color: '#b45309', marginBottom: 8 }}>{data.problems.profit.join(' · ')}</div>}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>{(data?.stores || []).map((s: string) => (
            <tr key={s}><td style={td}>{s}</td><td style={td}>
              <select value={mapOf[s] || ''} onChange={e => mapStore(s, e.target.value)}><option value="">— none —</option>{centers.map((c: any) => <option key={c.code} value={c.code}>{c.code} — {c.name}</option>)}</select>
            </td></tr>
          ))}</tbody>
        </table>
      ...
      {pc && (
        <div className="card" style={{ padding: 14 }}>
          ...
          {stmt?.pl && (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <tbody>
                {stmt.pl.sections.map((s: any) => (
                  <Fragment key={s.type}>
                    <tr><td style={{ ...td, fontWeight: 700 }}>{s.name}</td><td style={{ ...tdr, fontWeight: 700 }}>{fmt(s.subtotal)}</td></tr>
                    {s.lines.map((l: any) => <tr key={s.type + l.key}><td style={{ ...td, paddingLeft: 22 }}>{l.label}</td><td style={tdr}>{fmt(l.amount)}</td></tr>)}
                  </Fragment>
                ))}
