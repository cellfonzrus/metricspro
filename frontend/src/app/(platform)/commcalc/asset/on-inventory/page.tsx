...
type Data = { today: string; data_as_of: string | null; stores: StoreRow[]; totals: Totals }
...
export default function OnInventoryByStorePage() {
  ...
  function buildPayload(): ExportPayload {
    ...
    return {
      ...
      filename: `on-inventory-by-store${store ? '-' + store.replace(/[^a-z0-9]+/gi, '-').toLowerCase() : ''}`,
      sheets: [{ name: 'On-Inventory by Store', rows: sorted, columns: cols }],
    }
  }
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
                ...
                <tfoot>
                  <tr style={{ borderTop: '2px solid var(--border)', fontWeight: 700, background: 'var(--surface2)' }}>
                    ...
                    <td />
                    <td style={{ padding: '10px 12px', fontSize: 12, textAlign: 'right' }}>{data.totals.device_count.toLocaleString()}</td>
                    <td style={{ padding: '10px 12px', fontSize: 12, textAlign: 'right' }}>{fmt(data.totals.owed)}</td>
                    <td />
