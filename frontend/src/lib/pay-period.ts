// Tenant pay-period range helpers — THE one shared frontend period resolver (Phase W2, owner
// directive 2026-09-01: "the default period for hours approval must be the same period as the
// schedule / payroll / payroll tax / payroll expenses — one shared resolver, never five copies").
// PROMOTED from app/(platform)/storeops/lib/pay-period.ts (which now re-exports this file, so the
// original consumers are untouched) so surfaces OUTSIDE the storeops route tree (e.g. /hr/*) can
// adopt the same default without a cross-route relative import.
//
// (2026-07-25, owner: "the date range by default should be as
